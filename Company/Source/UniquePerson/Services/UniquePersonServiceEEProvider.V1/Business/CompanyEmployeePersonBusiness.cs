...
{
    public class CompanyEmployeePersonBusiness
    {
        private Core.Application.UniquePersonService.V1.UniquePersonServiceEEProvider coreProvider;

        public CompanyEmployeePersonBusiness()
        {
            coreProvider = new Core.Application.UniquePersonService.V1.UniquePersonServiceEEProvider();
        }

        internal List<CompanyEmployeePerson> GetCompanyEmployeePerson()
        {
            var imapper = ModelAssembler.CreateMapperEmployeePerson();
            var result = coreProvider.GetEmployeePersons();
            return imapper.Map<List<EmployeePerson>, List<CompanyEmployeePerson>>(result);
        }
    }
}
