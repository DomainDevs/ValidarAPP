...
{
    public class CompanyGroupAgentBusiness
    {
        private Core.Application.UniquePersonService.V1.UniquePersonServiceEEProvider coreProvider;

        public CompanyGroupAgentBusiness()
        {
            coreProvider = new Core.Application.UniquePersonService.V1.UniquePersonServiceEEProvider();
        }

        internal List<CompanyGroupAgent> GetCompanyGroupAgent()
        {
            var imapper = ModelAssembler.CreateMapperGroupAgent();
            var result = coreProvider.GetGroupAgent();
            return imapper.Map<List<GroupAgent>, List<CompanyGroupAgent>>(result);
        }
    }
}
