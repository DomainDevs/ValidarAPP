...
namespace Sistran.Company.Application.FidelityChangeAgentService.EEProvider.Business
{

    public class FidelityChangeAgentBusinessCia
    {
        BaseBusinessCia baseBusinessCia;
        /// <summary>
        /// Initializes a new instance of the <see cref="FidelityChangeAgentBusinessCia"/> class.
        /// </summary>
        public FidelityChangeAgentBusinessCia()
        {
        ...
        private CompanyPolicy ChangeAgentPolicy(CompanyPolicy companyPolicy)
        {
            try
            {
                ...
                if (companyPolicy.Id == 0)
                {
                    ...
                    companyFidelitys.AsParallel().ForAll(
                        x =>
                        {
                            x.Risk.Id = 0;
                ...
                {
                    companyFidelitys = DelegateService.FidelityService.GetCompanyFidelitiesByTemporalId(companyPolicy.Id);
                    if (companyFidelitys == null || companyFidelitys.Count < 1)
                    {
                        companyFidelitys = DelegateService.FidelityService.GetCompanyFidelitiesByPolicyId(companyPolicy.Endorsement.PolicyId);
                    }
                }
                ...
                if (companyPolicy != null && companyFidelitys?.Count > 0 && risks?.Count > 0)
                {
                    ConcurrentBag<string> errors = new ConcurrentBag<string>();
                    companyFidelitys.Where(a => a != null).AsParallel().ForAll(
                    z =>
                    {
                    ...
                    companyFidelitys.AsParallel().ForAll(item =>
                    {
                        try
                        {
                            item = DelegateService.fidelityModificationService.GetDataModification(item, CoverageStatusType.Included);
                            ...
                            item = DelegateService.FidelityService.CreateFidelityTemporal(item, false);
                    ...
                    if (companyFidelitys != null && companyFidelitys.Select(x => x.Risk).Any())
                    {

                        riskInfringementPolicies.AddRange(companyFidelitys.Select(x => x.Risk).SelectMany(x => x.InfringementPolicies).Where(m => m != null).ToList());

                        companyPolicy = baseBusinessCia.CalculatePolicyAmounts(companyPolicy, companyFidelitys.Select(x => x.Risk).ToList());
        ...
        public List<CompanyPolicy> CreateEndorsementChangeAgent(CompanyEndorsement companyEndorsement)
        {
            try
            {
                ...
                try
                {
                    ...
                    var createEndorsementChangeAgent = CreatefidelityEndorsementChangeAgent(companyEndorsement);
        ...
        /// <summary>
        /// Creates the fidelity endorsement change agent.
        /// </summary>
        /// <param name="companyEndorsement">The company endorsement.</param>
        /// <returns></returns>
        private CompanyPolicy CreatefidelityEndorsementChangeAgent(CompanyEndorsement companyEndorsement)
        {
            var companyPolicy = DelegateService.underwritingService.GetCompanyPolicyByTemporalId(companyEndorsement.TemporalId, false);
            List<LEM.CompanyFidelityRisk> fidelityRisks = DelegateService.FidelityService.GetCompanyFidelitiesByTemporalId(companyPolicy.Id);
            return DelegateService.FidelityService.CreateEndorsement(companyPolicy, fidelityRisks);
        ...
        private CompanyPolicy CancellationPolicy(CompanyEndorsement companyEndorsement)
        {
            ...
            var companyPolicy = DelegateService.endorsementfidelityCancellationService.CreateTemporalEndorsementCancellation(companyEndorsement);
            var fidelityRisks = DelegateService.FidelityService.GetCompanyFidelitiesByTemporalId(companyPolicy.Id);
            return DelegateService.FidelityService.CreateEndorsement(companyPolicy, fidelityRisks);
        }
    }
}
