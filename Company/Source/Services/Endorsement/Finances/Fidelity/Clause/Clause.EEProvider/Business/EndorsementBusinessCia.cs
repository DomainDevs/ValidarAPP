...
{
    using AutoMapper;
    using Sistran.Company.Application.FidelityClauseService.EEProvider.Assemblers;
    ...
    public class FidelityEndorsementBusinessCia
    {
                ...
                if (policy != null)
                {
                    ...
                    if (policy.Product.IsCollective)
                    {
                        if (companyEndorsement.DescriptionRisk != null)
                        {
                            ...
                            companyFidelitys = DelegateService.FidelityService.GetCompanyFidelitiesByEndorsementId(companyEndorsement.Id);
                            ...
                            LEM.CompanyFidelityRisk risk = GetDataModification(fidelity, policy, CoverageStatusType.NotModified);

                            List<LEM.CompanyFidelityRisk> fidelitys = DelegateService.FidelityService.GetCompanyFidelitiesByTemporalId(companyEndorsement.TemporalId);
        ...
        private CompanyPolicy CreateTempByEndorsement(CompanyEndorsement companyEndorsement)
        {
            ...
            if (policy != null)
            {
                ...
                foreach (LEM.CompanyFidelityRisk item in companyFidelitys)
                {
                    LEM.CompanyFidelityRisk risk = GetDataModification(item, policy, CoverageStatusType.NotModified);
        ...
        /// <returns></returns>
        private LEM.CompanyFidelityRisk GetDataModification(LEM.CompanyFidelityRisk risk, CompanyPolicy fidelityPolicy, CoverageStatusType coverageStatusType)
        {
            ...
            {
                List<CompanyCoverage> coverages = DelegateService.underwritingService.GetCompanyCoveragesByProductIdGroupCoverageIdPrefixId(fidelityPolicy.Product.Id, risk.Risk.GroupCoverage.Id, fidelityPolicy.Prefix.Id);
