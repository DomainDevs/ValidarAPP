...
{
    using AutoMapper;
    using Sistran.Company.Application.ThirdPartyLiabilityClauseService.EEProvider.Assemblers;
    ...
    public class ThirdPartyLiabilityEndorsementBusinessCia
    {
                ...
                if (policy != null)
                {
                    ...
                    if (policy.Product.IsCollective)
                    {
                        if (companyEndorsement.DescriptionRisk != null)
                        {
                            ...
                            companyTpls = DelegateService.tplService.GetCompanyThirdPartyLiabilitiesByEndorsementId(companyEndorsement.Id);
                            tpl = companyTpls.Where(x => x.LicensePlate == companyEndorsement.DescriptionRisk).FirstOrDefault();
                            ...
                            TPLEM.CompanyTplRisk risk = GetDataModification(tpl, policy, CoverageStatusType.NotModified);

                            List<TPLEM.CompanyTplRisk> tpls = DelegateService.tplService.GetThirdPartyLiabilitiesByTemporalId(companyEndorsement.TemporalId);

                          
                                risk.Risk.Policy = policy;
                                risk = DelegateService.tplService.CreateThirdPartyLiabilityTemporal(risk, true);
            ...
            if (policy != null)
            {
                ...
                if (policy.Product.IsCollective)
                {
                    if (companyEndorsement.DescriptionRisk != null)
                    {
                        ...
                        {
                            companyTpls = DelegateService.tplService.GetCompanyThirdPartyLiabilitiesByPolicyId(companyEndorsement.PolicyId);
                            companyTpls = companyTpls.Where(x => x.LicensePlate == companyEndorsement.DescriptionRisk).ToList();
                        }
                        else
                        {
                ...
                var mappper =  ModelAssembler.CreateMapCompanyClause();
                policy.Clauses = mappper.Map<List<Clause>, List<CompanyClause>>(DelegateService.underwritingService.GetClausesByEmissionLevelConditionLevelId(EmissionLevel.General, policy.Prefix.Id).Where(x => x.IsMandatory).ToList());
                ...
                foreach (TPLEM.CompanyTplRisk item in companyTpls)
                {
                    TPLEM.CompanyTplRisk risk = GetDataModification(item, policy, CoverageStatusType.NotModified);

                    risk.Risk.Policy = policy;
                    risk = DelegateService.tplService.CreateThirdPartyLiabilityTemporal(risk, false);
                    risk.Risk.Policy.InfringementPolicies.AddRange(risk.Risk.InfringementPolicies);
                }
                return policy;
            }
        ...
        /// <param name="tplPolicy">The tpl policy.</param>
        /// <param name="coverageStatusType">Type of the coverage status.</param>
        /// <returns></returns>
        private TPLEM.CompanyTplRisk GetDataModification(TPLEM.CompanyTplRisk risk, CompanyPolicy tplPolicy, CoverageStatusType coverageStatusType)
        {
            ...
            {
                List<CompanyCoverage> coverages = DelegateService.underwritingService.GetCompanyCoveragesByProductIdGroupCoverageIdPrefixId(tplPolicy.Product.Id, risk.Risk.GroupCoverage.Id, tplPolicy.Prefix.Id);
                ...
                foreach (CompanyCoverage item in coverages)
                {
                    ...
                    item.EndorsementType = tplPolicy.Endorsement.EndorsementType;
                    item.CurrentFrom = tplPolicy.CurrentFrom;
                    item.CurrentTo = tplPolicy.CurrentTo;
