...
using Sistran.Company.Application.ThirdPartyLiabilityRenewalService.EEProvider.Assemblers;
using Sistran.Company.Application.ThirdPartyLiabilityRenewalService.EEProvider.Resources;
using Sistran.Company.Application.ThirdPartyLiabilityRenewalService.EEProvider.Services;
...
namespace Sistran.Company.Application.ThirdPartyLiabilityRenewalService.EEProvider.Business
{
    public class RenewalBusinessCia
    {
        ...
        public CompanyPolicy CreateTemporal(CompanyPolicy companyPolicy)
        {
            try
            {
                ...
                if (policy != null)
                {
                    ...
                    if (id > 0)
                    {
                        companyTpls = DelegateService.tplService.GetThirdPartyLiabilitiesByTemporalId(id);
                    }
                    else
                    {
                    ...
                    if (companyTpls != null)
                    {
                        ...
                        if (policy.Product.IsCollective && companyPolicy.Endorsement.IsUnderIdenticalConditions)
                        {
                            ...
                            TP.Parallel.ForEach(companyTpls, item =>
                            {
                                ...
                                CompanyRisk risk = Createtpl(item);
                                risks.Add(risk);
                            });
                        ...
                        else
                        {
                            ...
                            TP.Parallel.ForEach(companyTpls,  item =>
                            {
                                try
                                {
                                    if (item != null)
                                    {
                                        var risk = new TPLEM.CompanyTplRisk();
                                        lock (objectLock)
                                        {
        ...
        /// <exception cref="Exception">
        /// </exception>
        private TPLEM.CompanyTplRisk GetDataRenewal(TPLEM.CompanyTplRisk risk)
        {
            ...
            risk = DelegateService.tplService.QuotateThirdPartyLiability(risk, true, true);
        ...
        private CompanyRisk Createtpl(TPLEM.CompanyTplRisk item)
        {
            item.Risk.IsPersisted = true;
            TPLEM.CompanyTplRisk tpl = item;

            tpl = GetDataRenewal(tpl);
            tpl.Risk.RiskId = 0;

            tpl = DelegateService.tplService.CreateThirdPartyLiabilityTemporal(tpl, false);
