...
namespace Sistran.Company.Application.VehicleReversionService.EEProvider.DAOs
{
    using System.Collections.Generic;
    using Core.Application.AuthorizationPoliciesServices.Models;
    using Core.Application.UnderwritingServices.Models;
    ...
    public class ReversionDAOCia
    {
        ...
        {
            policy.InfringementPolicies = !clearPolicies ? DelegateService.underwritingService.ValidateAuthorizationPolicies(policy) : new List<PoliciesAut>();
            ...
            if (policy.InfringementPolicies == null || policy.InfringementPolicies.Count == 0)
            {
                ...
                {
                    parameters[2] = new NameValue("@CONDITION_TEXT", DBNull.Value, DbType.AnsiString);
                }
                else
                {
                ...
                {
                    parameters[4] = new NameValue("@ANNOTATIONS", DBNull.Value, DbType.AnsiString);
                }
                else
                {
