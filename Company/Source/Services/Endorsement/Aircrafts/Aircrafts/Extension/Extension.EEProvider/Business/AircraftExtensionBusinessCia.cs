...
using Sistran.Company.Application.AircraftExtensionService.EEProvider.Resources;
using Sistran.Company.Application.Aircrafts.AircraftBusinessService.Models.Base;
using AMA = Sistran.Company.Application.AircraftExtensionService.EEProvider.AutoMapperAssembler;
...
namespace Sistran.Company.Application.AircraftExtensionService.EEProvider.Business
{
    class AircraftExtensionBusinessCia
    {
        ...
        /// </summary>
        public AircraftExtensionBusinessCia()
        {
                ...
                if (companyPolicy?.Endorsement?.TemporalId > 0)
                {
                    ...
                    if (policy != null)
                    {
                        ...
                        if (companyAircrafts != null && companyAircrafts.Any())
                        {
                            ...
                            risks = CreateExtension(companyAircrafts);
                        }
                        else
                        {
                            companyAircrafts = DelegateService.aircraftService.GetCompanyAircraftsByPolicyId(companyPolicy.Endorsement.PolicyId);
                            ...
                            {
                                throw new Exception("Error Obteniendo aircraftes");
                            }
                        }
        ...
        /// Creates Extension.
        /// </summary>
        /// <param name="companyAircrafts">The company endorsement.</param>
        /// <returns></returns>
        private List<CompanyRisk> CreateExtension(List<CompanyAircraft> companyAircrafts)
        {
            if (companyAircrafts != null && companyAircrafts.Any())
            {
                List<CompanyRisk> risks = new List<CompanyRisk>();
                ...
                if ((bool)companyAircrafts.First()?.Risk?.Policy.Product.IsCollective)
                {
                    if (companyAircrafts.First().Risk.Policy.Endorsement.TemporalId > 0)
                    {
                        companyAircrafts = DelegateService.aircraftService.GetCompanyAircraftsByTemporalId(companyAircrafts.First().Risk.Policy.Endorsement.TemporalId);
                        ...
                        var aircraftPolicy = provider.CalculatePolicyAmounts(risks.First().Policy, risks);

                        aircraftPolicy = DelegateService.underwritingService.CreatePolicyTemporal(aircraftPolicy, false);

                        return risks;
                    }
                    else
                    {
                        companyAircrafts = DelegateService.aircraftService.GetCompanyAircraftsByPolicyId(companyAircrafts.First().Risk.Policy.Endorsement.PolicyId);
                        TP.Parallel.ForEach(companyAircrafts, item =>
                        {
                            item.Risk.IsPersisted = true;
                            var risk = GetDataExtension(item);
            ...
            {
                throw new Exception("No existen Aircraftes");
            }
        ...
        /// <returns></returns>
        private CompanyAircraft GetDataExtension(CompanyAircraft risk)
        {
            ...
            risk = DelegateService.aircraftService.QuotateCompanyAircraft(risk, false, false);
