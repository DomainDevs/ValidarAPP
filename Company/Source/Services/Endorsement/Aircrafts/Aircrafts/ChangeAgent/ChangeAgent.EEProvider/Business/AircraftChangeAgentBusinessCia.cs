...
namespace Sistran.Company.Application.AircraftChangeAgentService.EEProvider.Business
{

    public class AircraftChangeAgentBusinessCia
    {
        BaseBusinessCia baseBusinessCia;
        /// <summary>
        /// Initializes a new instance of the <see cref="AircraftChangeAgentBusinessCia"/> class.
        /// </summary>
        public AircraftChangeAgentBusinessCia()
        {
        ...
        private CompanyPolicy ChangeAgentPolicy(CompanyPolicy companyPolicy)
        {
            try
            {
                ...
                if (companyPolicy.Id == 0)
                {
                    ...
                    companyAircrafts.AsParallel().ForAll(
                        x =>
                        {
                            x.Risk.Id = 0;
                ...
                {
                    companyAircrafts = DelegateService.aircraftService.GetCompanyAircraftsByTemporalId(companyPolicy.Id);
                    if (companyAircrafts == null && companyAircrafts.Count < 1)
                    {
                        companyAircrafts = DelegateService.aircraftService.GetCompanyAircraftsByPolicyId(companyPolicy.Endorsement.PolicyId);
                    }
                }
                ...
                if (companyPolicy != null && companyAircrafts?.Count > 0 && risks?.Count > 0)
                {
                    ...
                    companyAircrafts.AsParallel().ForAll(item =>
                    {
                        try
                        {
                            item = DelegateService.aircraftModificationService.GetDataModification(item, CoverageStatusType.Included);
                            ...
                            item = DelegateService.aircraftService.CreateCompanyAircraftTemporal(item);
                    ...
                    if (companyAircrafts != null && companyAircrafts.Select(x => x.Risk).Any())
                    {

                        riskInfringementPolicies.AddRange(companyAircrafts.Select(x => x.Risk).SelectMany(x => x.InfringementPolicies).Where(m => m != null).ToList());

                        companyPolicy = baseBusinessCia.CalculatePolicyAmounts(companyPolicy, companyAircrafts.Select(x => x.Risk).ToList());
                ...
                {
                    throw new Exception("Aircraft no Encontrados");
                }
        ...
        public List<CompanyPolicy> CreateEndorsementChangeAgent(CompanyEndorsement companyEndorsement)
        {
            try
            {
                ...
                try
                {
                    ...
                    var createEndorsementChangeAgent = CreateAircraftEndorsementChangeAgent(companyEndorsement);
        ...
        /// <summary>
        /// Creates the aircraft endorsement change agent.
        /// </summary>
        /// <param name="companyEndorsement">The company endorsement.</param>
        /// <returns></returns>
        private CompanyPolicy CreateAircraftEndorsementChangeAgent(CompanyEndorsement companyEndorsement)
        {
            var companyPolicy = DelegateService.underwritingService.GetCompanyPolicyByTemporalId(companyEndorsement.TemporalId, false);
            List<CompanyAircraft> aircrafts = DelegateService.aircraftService.GetCompanyAircraftsByTemporalId(companyPolicy.Id);
            return DelegateService.aircraftService.CreateEndorsement(companyPolicy, aircrafts);
        }
        ...
        private CompanyPolicy CancellationPolicy(CompanyEndorsement companyEndorsement)
        {
            ...
            var companyPolicy = DelegateService.endorsementAircraftCancellationService.CreateTemporalEndorsementCancellation(companyEndorsement);
            var aircrafts = DelegateService.aircraftService.GetCompanyAircraftsByTemporalId(companyPolicy.Id);
            return DelegateService.aircraftService.CreateEndorsement(companyPolicy, aircrafts);
        }
    }
}
