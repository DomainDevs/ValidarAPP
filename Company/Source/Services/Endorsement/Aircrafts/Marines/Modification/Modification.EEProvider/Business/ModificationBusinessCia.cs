...
using Sistran.Company.Application.MarineModificationService.EEProvider.Resources;
...
namespace Sistran.Company.Application.MarineModificationService.EEProvider.Business
{
    public class ModificationBusinessCia
    {
                ...
                if (policy != null)
                {
                    ...
                    if (policy.Product.IsCollective)
                    {
                        if (companyEndorsement.DescriptionRisk != null)
                        {
                            ...
                            companymarines = Services.DelegateService.marineService.GetCompanyMarinesByEndorsementId(companyEndorsement.Id);
                            marine = companymarines.Where(x => x.UseDescription + " - " + x.BoatName == companyEndorsement.DescriptionRisk).FirstOrDefault();
                            ...
                            marine.Risk.Policy = policy;
                            CompanyMarine risk = GetDataModification(marine, CoverageStatusType.NotModified);

                            List<CompanyMarine> marines = Services.DelegateService.marineService.GetCompanyMarinesByTemporalId(companyEndorsement.TemporalId);

                            risk.Risk.Policy = policy;
                            risk = Services.DelegateService.marineService.CreateCompanyMarineTemporal(risk);
                ...
                if (policy != null)
                {
                    ...
                    if (policy.Product.IsCollective)
                    {
                        if (companyEndorsement.DescriptionRisk != null)
                        {
                            ...
                            {
                                companyMarines = Services.DelegateService.marineService.GetCompanyMarinesByPolicyId(companyEndorsement.PolicyId);
                            }
                            else
                            {
        ...
        /// <returns></returns>
        public CompanyMarine GetDataModification(CompanyMarine risk, CoverageStatusType coverageStatusType)
        {
