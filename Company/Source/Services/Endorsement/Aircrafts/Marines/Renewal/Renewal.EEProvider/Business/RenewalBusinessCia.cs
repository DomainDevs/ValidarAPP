...
using Sistran.Company.Application.MarineRenewalService.EEProvider.Resources;
using Sistran.Company.Application.MarineRenewalService.EEProvider.Services;
...
namespace Sistran.Company.Application.MarineRenewalService.EEProvider.Business
{
    public class RenewalBusinessCia
    {
        ...
        public CompanyPolicy CreateTemporal(CompanyPolicy companyPolicy)
        {
            try
            {
                ...
                if (policy != null)
                {
                    ...
                    if (id > 0)
                    {
                        companyMarines = DelegateService.marineService.GetCompanyMarinesByTemporalId(id);
                    }
                    else
                    {
                    ...
                    if (companyMarines != null)
                    {
                        ...
                        if (policy.Product.IsCollective && companyPolicy.Endorsement.IsUnderIdenticalConditions)
                        {
                            ...
                            Parallel.ForEach(companyMarines, ParallelHelper.DebugParallelFor(), item =>
                            {
                                item.Risk.Policy = policy;
                                CompanyRisk risk = Createmarine(item);
                                risks.Add(risk);
                            });
        ...
        /// <exception cref="Exception">
        /// </exception>
        private CompanyMarine GetDataRenewal(CompanyMarine risk)
        {
            ...
            risk = DelegateService.marineService.QuotateCompanyMarine(risk, true, true);
        ...
        private CompanyRisk Createmarine(CompanyMarine item)
        {
            item.Risk.IsPersisted = true;
            CompanyMarine marine = item;

            marine = GetDataRenewal(marine);
            marine.Risk.RiskId = 0;

            marine = DelegateService.marineService.CreateCompanyMarineTemporal(marine);
