...
using Sistran.Company.Application.MarineExtensionService.EEProvider.Resources;
using Sistran.Company.Application.Marines.MarineBusinessService.Models.Base;
using AMA = Sistran.Company.Application.MarineExtensionService.EEProvider.AutoMapperAssembler;
...
namespace Sistran.Company.Application.MarineExtensionService.EEProvider.Business
{
    class MarineExtensionBusinessCia
    {
        ...
        /// </summary>
        public MarineExtensionBusinessCia()
        {
                ...
                if (companyPolicy?.Endorsement?.TemporalId > 0)
                {
                    ...
                    if (policy != null)
                    {
                        ...
                        if (companyMarines != null && companyMarines.Any())
                        {
                            ...
                            companyMarines.AsParallel().ForAll(x => x.Risk.Policy = policy);
                            risks = CreateExtension(companyMarines);
                        }
                        else
                        {
                            companyMarines = DelegateService.marineService.GetCompanyMarinesByPolicyId(companyPolicy.Endorsement.PolicyId);
                            ...
                            {
                                throw new Exception("Error Obteniendo marinees");
                            }
                        }
        ...
        /// Creates Extension.
        /// </summary>
        /// <param name="companyMarines">The company endorsement.</param>
        /// <returns></returns>
        private List<CompanyRisk> CreateExtension(List<CompanyMarine> companyMarines)
        {
            if (companyMarines != null && companyMarines.Any())
            {
                List<CompanyRisk> risks = new List<CompanyRisk>();
                ...
                if ((bool)companyMarines.First()?.Risk?.Policy.Product.IsCollective)
                {
                    if (companyMarines.First().Risk.Policy.Endorsement.TemporalId > 0)
                    {
                        companyMarines = DelegateService.marineService.GetCompanyMarinesByTemporalId(companyMarines.First().Risk.Policy.Endorsement.TemporalId);
                        TP.Parallel.ForEach(companyMarines, item =>
                        {
                            item.Risk.IsPersisted = true;
                            var risk = GetDataExtension(item);
                            risk = DelegateService.marineService.CreateCompanyMarineTemporal(risk);
                            risk.Risk.Policy.InfringementPolicies.AddRange(risk.Risk.InfringementPolicies);
                            risks.Add(risk.Risk);
                        });

                        var marinePolicy = provider.CalculatePolicyAmounts(risks.First().Policy, risks);

                        marinePolicy = DelegateService.underwritingService.CreatePolicyTemporal(marinePolicy, false);

                        return risks;
                    }
                    else
                    {
                        companyMarines = DelegateService.marineService.GetCompanyMarinesByPolicyId(companyMarines.First().Risk.Policy.Endorsement.PolicyId);
                        TP.Parallel.ForEach(companyMarines, item =>
                        {
                            item.Risk.IsPersisted = true;
                            var risk = GetDataExtension(item);
            ...
            {
                throw new Exception("No existen Marinees");
            }
        ...
        /// <returns></returns>
        private CompanyMarine GetDataExtension(CompanyMarine risk)
        {
            ...
            risk = DelegateService.marineService.QuotateCompanyMarine(risk, false, false);
