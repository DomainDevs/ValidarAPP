...
using Sistran.Company.Application.TransportModificationService.EEProvider.Resources;
...
namespace Sistran.Company.Application.TransportModificationService.EEProvider.Business
{
    public class ModificationBusinessCia
    {
                ...
                if (policy != null)
                {
                    ...
                    if (policy.PolicyOrigin.Equals(PolicyOrigin.Collective))
                    {
                        if (companyEndorsement.DescriptionRisk != null)
                        {
                            ...
                            /// jhgomez
                            /// companytransports = Services.DelegateService.transportService.GetCompanyTransportsByEndorsementId(companyEndorsement.Id);
                            policy.Endorsement.EndorsementType = EndorsementType.Modification;
                            policy.Endorsement.EndorsementTypeDescription = EnumHelper.GetItemName<EndorsementType>(policy.Endorsement.EndorsementType);
                            ...
                            transport.Risk.Policy = policy;
                            CompanyTransport risk = GetDataModification(transport, CoverageStatusType.NotModified);

                            List<CompanyTransport> transports = Services.DelegateService.transportService.GetCompanyTransportsByTemporalId(companyEndorsement.TemporalId);

                            /// jhgomez
                            /// List<string> message = Services.DelegateService.transportService.ExistsRisk(transports, 0, companyEndorsement.DescriptionRisk, "", "", EndorsementType.Modification, policy.Endorsement.Id);
                            List<string> message = new List<string>();

                            if (message.Count == 0)
                            {
                                risk.Risk.Policy = policy;
                                risk = Services.DelegateService.transportService.CreateCompanyTransportTemporal(risk);
                ...
                if (policy != null)
                {
                    ...
                    if (policy.PolicyOrigin.Equals(PolicyOrigin.Collective))
                    {
                        if (companyEndorsement.DescriptionRisk != null)
                        {
                            ...
                            {
                                companyTransports = Services.DelegateService.transportService.GetCompanyTransportsByPolicyId(companyEndorsement.PolicyId);
                            }
                            else
                            {
                    ...
                    policy.Endorsement.EndorsementReasonId = companyEndorsement.EndorsementReasonId;
                    policy.Endorsement.EndorsementType = EndorsementType.Modification;
                    //Cuando viene de R1 no recupera de la pending operation y las coberturas ya vienen con prima 
                    foreach (var item in companyTransports)
                    {
                        item.Risk.Coverages.ForEach(x => { x.PremiumAmount = 0; });
                    }
                    policy.Endorsement.EndorsementTypeDescription = EnumHelper.GetItemName<EndorsementType>(policy.Endorsement.EndorsementType);
                    ...
                    if (companyTransports != null && companyTransports.Count > 0)
                    {
                        foreach (var item in companyTransports)
                        {
                            if (item.AdjustPeriod.Id != (int)EnumsAdjustmentPeriod.Annual)
                            {
                                throw new BusinessException(Errors.ErrorAdjustPeriodAnnual);
                            }
                        }
                    }
                    return policy;
        ...
        /// <returns></returns>
        public CompanyTransport GetDataModification(CompanyTransport risk, CoverageStatusType coverageStatusType)
        {
