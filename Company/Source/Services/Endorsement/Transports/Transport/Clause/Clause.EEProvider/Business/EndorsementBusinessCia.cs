...
namespace Sistran.Company.Application.TransportClauseService.EEProvider.Business
{
    ...
    /// Endosos Autos
    /// </summary>
    public class TransportEndorsementBusinessCia
    {
        ...
        /// </summary>
        public TransportEndorsementBusinessCia()
        {
        ...
        private CompanyPolicy CreateTempByTempId(CompanyEndorsement companyEndorsement)
        {
            try
            {
                ...
                //            /// jhgomez
                //            /// companyTransports = DelegateService.transportService.GetCompanyTransportsByEndorsementId(companyEndorsement.Id);
                //            /// transport = companyTransports.Where(x => x.LicensePlate == companyEndorsement.DescriptionRisk).FirstOrDefault();
                ...
                //            CompanyTransport risk = GetDataModification(transport, policy, CoverageStatusType.NotModified);

                //            List<CompanyTransport> transports = DelegateService.transportService.GetCompanyTransportsByTemporalId(companyEndorsement.TemporalId);

                //            /// jhgomez
                //            List<string> message = new List<string>();
                //            /// List<string> message = DelegateService.transportService.ExistsRisk(transports, 0, companyEndorsement.DescriptionRisk, "", "", EndorsementType.Modification, policy.Endorsement.Id);

                //            if (message.Count == 0)
                //            {
                //                risk.Risk.Policy = policy;
                //                risk = DelegateService.transportService.CreateCompanyTransportTemporal(risk, true);
        ...
        private CompanyPolicy CreateTempByEndorsement(CompanyEndorsement companyEndorsement)
        {
            ...
            //                /// jhgomez
            //                /// companyTransports = companyTransports.Where(x => x.LicensePlate == companyEndorsement.DescriptionRisk).ToList();
            //            }
            //            else
            //            {
            ...
            //    foreach (CompanyTransport item in companyTransports)
            //    {
            //        CompanyTransport risk = GetDataModification(item, policy, CoverageStatusType.NotModified);

            //        risk.Risk.Policy = policy;
            //        risk = DelegateService.transportService.CreateCompanyTransportTemporal(risk, false);
            //        risk.Risk.Policy.InfringementPolicies.AddRange(risk.Risk.InfringementPolicies);
            //    }
            //    return policy;
            //}
        ...
        /// <param name="transportPolicy">The transport policy.</param>
        /// <param name="coverageStatusType">Type of the coverage status.</param>
        /// <returns></returns>
        private CompanyTransport GetDataModification(CompanyTransport risk, CompanyPolicy transportPolicy, CoverageStatusType coverageStatusType)
        {
            /// jhgomez
            ...
            //{
            //    List<CompanyCoverage> coverages = DelegateService.underwritingService.GetCompanyCoveragesByProductIdGroupCoverageIdPrefixId(transportPolicy.Product.Id, risk.Risk.GroupCoverage.Id, transportPolicy.Prefix.Id);
            ...
            //        item.EndorsementType = transportPolicy.Endorsement.EndorsementType;
            //        item.CurrentFrom = transportPolicy.CurrentFrom;
            //        item.CurrentTo = transportPolicy.CurrentTo;
