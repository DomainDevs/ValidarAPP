using Sistran.Company.Application.PropertyReversionService.EEProvider.Business;
using Sistran.Company.Application.ReversionEndorsement.EEProvider;
using Sistran.Company.Application.UnderwritingServices;
using Sistran.Company.Application.UnderwritingServices.Models;
using Sistran.Core.Framework.BAF;
using System;
using Sistran.Core.Application.PropertyEndorsementReversionService.EEProvider;
using Sistran.Core.Application.PropertyEndorsementReversionService;
using Sistran.Company.Application.PropertyReversionService.EEProvider.Assemblers;

namespace Sistran.Company.Application.PropertyReversionService.EEProvider
{
    public class PropertyReversionServiceEEProvider : CiaReversionEndorsementEEProvider, IPropertyReversionServiceCia
    {
        /// <summary>
        /// Crear temporal de anulacion
        /// </summary>
        /// <param name="policy">Modelo policy</param>
        /// <returns>Id temporal</returns>
        public CompanyPolicy CreateEndorsementReversion(CompanyEndorsement policy, bool clearPolicies)
        {
            try
            {
                PropertyReversionBusinessCia propertyReversionBusinessCia = new PropertyReversionBusinessCia();
                CompanyPolicy companyPolicy = propertyReversionBusinessCia.CreateTemporal(policy, clearPolicies);

                if (companyPolicy.InfringementPolicies == null || companyPolicy.InfringementPolicies.Count == 0)
                {
                    CreateEndorsementWorkFlow(companyPolicy.Endorsement.PolicyId, companyPolicy.Endorsement.Id, companyPolicy.TicketNumber.ToString(), Convert.ToDateTime(companyPolicy.TicketDate));
                    DelegateService.underwritingService.SaveTextLarge(companyPolicy.Endorsement.PolicyId, companyPolicy.Endorsement.Id);
                }
                return companyPolicy;

            }
            catch (System.Exception ex)
            {

                throw new BusinessException(ex.Message, ex);
            }
        }

       
    }
}