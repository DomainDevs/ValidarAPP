...
{
    using AutoMapper;
    using Sistran.Company.Application.LiabilityClauseService.EEProvider.Assemblers;
    ...
    public class LiabilityEndorsementBusinessCia
    {
                ...
                if (policy != null)
                {
                    ...
                    if (policy.Product.IsCollective)
                    {
                        if (companyEndorsement.DescriptionRisk != null)
                        {
                            ...
                            companyLibialitys = DelegateService.LibialityService.GetCompanyLiabilitiesByEndorsementId(companyEndorsement.Id);
                            ...
                            LEM.CompanyLiabilityRisk risk = GetDataModification(libiality, policy, CoverageStatusType.NotModified);

                            List<LEM.CompanyLiabilityRisk> libialitys = DelegateService.LibialityService.GetCompanyLiabilitiesByTemporalId(companyEndorsement.TemporalId);
                                                               
                                risk.Risk.Policy = policy;
                                risk = DelegateService.LibialityService.CreateLiabilityTemporal(risk, true);
        ...
        private CompanyPolicy CreateTempByEndorsement(CompanyEndorsement companyEndorsement)
        {
            ...
            if (policy != null)
            {
                ...
                foreach (LEM.CompanyLiabilityRisk item in companyLibialitys)
                {
                    LEM.CompanyLiabilityRisk risk = GetDataModification(item, policy, CoverageStatusType.NotModified);
        ...
        /// <returns></returns>
        private LEM.CompanyLiabilityRisk GetDataModification(LEM.CompanyLiabilityRisk risk, CompanyPolicy liabilityPolicy, CoverageStatusType coverageStatusType)
        {
            ...
            {
                List<CompanyCoverage> coverages = DelegateService.underwritingService.GetCompanyCoveragesByProductIdGroupCoverageIdPrefixId(liabilityPolicy.Product.Id, risk.Risk.GroupCoverage.Id, liabilityPolicy.Prefix.Id);
