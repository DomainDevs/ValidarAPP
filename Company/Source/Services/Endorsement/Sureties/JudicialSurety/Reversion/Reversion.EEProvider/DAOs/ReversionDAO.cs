...
namespace Sistran.Company.Application.JudicialSuretyReversionService.EEProvider.DAOs
{
    public class ReversionDAOCompany
    {
        ...
        public CompanyPolicy CreateEndorsementReversion(CompanyPolicy policy, bool clearPolicies)
        {
            ...
            if (policy.InfringementPolicies == null || policy.InfringementPolicies.Count == 0)
            {
                ...
                {
                    dataTable = pdb.ExecuteSPDataTable("TMP.CANCELLATION_ENDORSEMENT_JUDICIAL_SURETY", parameters);
                }
                ...
                if (dataTable != null && dataTable.Rows.Count > 0)
                {
                    if (Convert.ToBoolean(dataTable.Rows[0][0]))
                    {
                        ...
                        int PolicyIdReversion = Convert.ToInt32(dataTable.Rows[0][3]);

                        DelegateService.underwritingService.DeleteTemporalByOperationId(policy.Id, 0, 0, 0);
                        if (DelegateService.commonService.GetParameterByDescription("UND_ENABLED_REINSURANCE").BoolParameter.GetValueOrDefault())
                        {
                            if (PolicyIdReversion != policy.Endorsement.PolicyId)
                            {
                                int endorsementIdCancelation = Convert.ToInt32(dataTable.Rows[0][4]);
                                policy.IsReinsured = DelegateService.underwritingReinsuranceWorkerIntegration.ReinsuranceIssue(PolicyIdReversion, policy.Endorsement.Id, policy.UserId) > 0;
                                policy.IsReinsured = DelegateService.underwritingReinsuranceWorkerIntegration.ReinsuranceIssue(policy.Endorsement.PolicyId, endorsementIdCancelation, policy.UserId) > 0;
                            }
                            else
                            {
                                policy.IsReinsured = DelegateService.underwritingReinsuranceWorkerIntegration.ReinsuranceIssue(policy.Endorsement.PolicyId, policy.Endorsement.Id, policy.UserId) > 0;
                            }
        ...
        public List<PoliciesAut> ValidateAuthorizationPolicies(CompanyPolicy policy)
        {
            ...
            Rules.Facade facade = DelegateService.underwritingService.CreateFacadeGeneral(policy);
