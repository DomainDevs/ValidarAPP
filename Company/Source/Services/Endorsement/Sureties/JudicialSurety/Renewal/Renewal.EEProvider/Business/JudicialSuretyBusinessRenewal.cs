...
using Sistran.Company.Application.JudicialSuretyRenewalService.EEProvider.Services;
...
using Sistran.Company.Application.JudicialSuretyRenewalService.EEProvider.Assemblers;
using Sistran.Company.Application.JudicialSuretyRenewalService.EEProvider.Resources;

namespace Sistran.Company.Application.JudicialSuretyRenewalService.EEProvider.Business
{
    class JudicialSuretyBusinessRenewal
    {
        ...
        /// </summary>
        public JudicialSuretyBusinessRenewal()
        {
        ...
        public CompanyPolicy CreateTemporal(CompanyPolicy companyPolicy)
        {
            try
            {
                ...
                if (policy != null)
                {
                    ...
                    List<CompanyJudgement> companySuretys = new List<CompanyJudgement>();
                    if (id > 0)
                    {
                        companySuretys = DelegateService.JudicialsuretyService.GetCompanyJudgementsByTemporalId(id);
        ...
        /// <exception cref="Exception">
        /// </exception>
        private CompanyJudgement GetDataRenewal(CompanyJudgement risk, CompanyPolicy policy)
        {
            ...
            risk = DelegateService.JudicialsuretyService.QuotateCompanyJudgement(risk, true, true);
        ...
        private CompanyRisk CreateSurety(CompanyJudgement item, CompanyPolicy policy)
        {
            item.Risk.IsPersisted = true;
            CompanyJudgement surety = item;

            surety = GetDataRenewal(surety, policy);
            surety.Risk.RiskId = 0;

            surety = DelegateService.JudicialsuretyService.CreateJudgementTemporal(surety, false);
