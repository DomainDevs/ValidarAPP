...
using Sistran.Company.Application.JudicialSuretyChangePolicyHolderService.EEProvider.Assemblers;
...
namespace Sistran.Company.Application.JudicialSuretyChangePolicyHolderService.EEProvider.Business
{
    public class JudicialSuretyChangePolicyHolderBusinessCia
    {
        ...
        /// </summary>
        public JudicialSuretyChangePolicyHolderBusinessCia()
        {
        ...
        private CompanyPolicy ChangePolicyHolderPolicy(CompanyPolicy companyPolicy)
        {
            try
            {
                ...
                if (companyPolicy.Id == 0)
                {
                    companyPolicy.TemporalType = TemporalType.Endorsement;
                    companyPolicy.TemporalTypeDescription = EnumHelper.GetItemName<TemporalType>(TemporalType.Endorsement);
                    companySuretys = DelegateService.suretyService.GetCompanyJudicialSuretyByPolicyId(companyPolicy.Endorsement.PolicyId);
                }
                else
                {
                    companySuretys = DelegateService.suretyService.GetCompanyJudgementsByTemporalId(companyPolicy.Id);
                }
                ...
                if (companySuretys != null && companySuretys.Count > 0 && risks != null)
                {
                    ...
                    companySuretys.AsParallel().ForAll(item =>
                    {
                        ...
                        item = DelegateService.suretyService.CreateJudgementTemporal(item, false);
                    });
        ...
        public List<CompanyPolicy> CreateEndorsementChangePolicyHolder(CompanyChangePolicyHolder companyChangePolicyHolder, bool clearPolicies)
        {
            try
            {
                ...
                if (!clearPolicies)
                {
                    List<CompanyJudgement> suretys = DelegateService.suretyService.GetCompanyJudgementsByTemporalId(companyPolicy.Id);
        ...
        private CompanyPolicy CancellationPolicy(CompanyEndorsement companyEndorsement)
        {
            ...
            var suretys = DelegateService.suretyService.GetCompanyJudgementsByTemporalId(companyPolicy.Id);
            companyPolicy.InfringementPolicies = new List<PoliciesAut>();
            suretys.ForEach(x => x.Risk.InfringementPolicies = new List<PoliciesAut>());
            return DelegateService.suretyService.CreateEndorsement(companyPolicy, suretys);
        }
