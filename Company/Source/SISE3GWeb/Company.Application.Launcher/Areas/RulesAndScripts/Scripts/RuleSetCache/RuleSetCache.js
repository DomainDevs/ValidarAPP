...
class RulesSetCache extends Uif2.Page {
    ...
    static RecordRuleNode() {
        $.UifDialog('confirm', { 'message': Resources.Language.SurepublishRuleSetLoad, 'title': AppResources.Confirmation }, function (result) {
            if (result === true) {
                RequestRulesSetCache.RecordRuleNode().done((data) => {
                    if (data.success) {
                        $.UifNotify("show", { 'type': "info", 'message': Resources.Language.NewRuleVersion, 'autoclose': false });
                    }
                    else {
