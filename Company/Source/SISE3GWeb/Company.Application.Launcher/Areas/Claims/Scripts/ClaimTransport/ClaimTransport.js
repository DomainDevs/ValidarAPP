class ClaimTransport extends Uif2.Page {
    getInitialState() {
        ...
        ClaimTransport.InitialTransport();
    }

    bindEvents() {
        $("#btnSaveClaim").on('click', ClaimTransport.CreateClaimTransport);

        $('#tableRiskTransport').on('rowSelected', function (event, data, position) {
            ClaimEstimation.GetCoveragesByRiskIdOccurrenceDateCompanyParticipationPercentage(data);
        });
        
        $('#tableRiskTransport').on('rowDeselected', function (event, data, position) {
            ClaimEstimation.DeselectedRisk(data);
        });

        $('#tableSubSinisterTransport').on('rowEdit', ClaimEstimation.EditSubClaims);

    }

    static InitialTransport() {
        if (noticeToClaim == null && modelSearchCriteria.claimId == null && modelSearchCriteria.number == null) {
            ClaimTransport.GetPrefixesByCoveredRiskType();
        }
        Claim.GetBranches();
        ClaimTransport.DoConvertNoticeTransportToClaim();
    }

    static GetPrefixesByCoveredRiskType(callback) {
        ClaimTransportRequest.GetPrefixesByCoveredRiskType().done(function (data) {
            if (data.success) {
                if (callback)
    ...
    static DoConvertNoticeTransportToClaim() {
    ...
    static GetRisksByEndorsementId(endorsementId) {
        ClaimTransportRequest.GetRisksByEndorsementId(endorsementId).done(function (data) {
            if (data.success) {
                $("#tableRiskTransport").UifDataTable({ sourceData: data.result });
    ...
    static preLoadClaimTransport() {
        ...
        ClaimRequest.GetClaimByClaimId(modelSearchCriteria.claimId).done(function (response) {
            if (response.success) {
                ...
                $("#transportClaimNumberEdit").show();
                $("#transportClaimNumberEdit").text($("#transportClaimNumberEdit").text() + response.result.Number);
    ...
    static CreateClaimTransport() {
        ...
        if (policyConsulted && Claim.ValidateTableSummaryClaim()) {
            lockScreen();
            var claimTransportyDTO = BuildExecuteClaim();

            ClaimTransportRequest.ExecuteClaimOperations(claimTransportyDTO).done(function (data) {
                if (data.success) {
