...
var conceptsCoverage = null;
var EditSubLineBusiness = null;
var currentOriginalLimitAmount = 0;
var currentOriginalSubLimitAmount = 0;
var riskData = null;
let CoveragesListAircraft = [];
var isNewInsuredObject = true;

class RiskAircraftCoverage extends Uif2.Page {
    getInitialState() {
        riskControllerCoverage = 'RiskAircraftCoverage';
        if (glbRisk == null) {
            glbRisk = { Id: 0, Object: "RiskAircraftCoverage", formAircraftCoverage: "#formAircraftCoverage", RecordScript: false, Class: RiskAircraftCoverage };
        }
        ...
        RiskAircraftCoverage.GetInsuredRateTypes();
        RiskAircraftCoverage.GetCalculationTypes();
        RiskAircraftCoverage.GetRateTypes();
        RiskAircraftCoverage.LoadTemporal(glbPolicy);

        if (IsEditing) {
            if (!RiskAircraftCoverage.GetTemporalCoveragesByRiskIdInsuredObjectId(glbRisk.Id, glbCoverage.CoverageId)) {
                RiskAircraftCoverage.GetInsuredObjectsByProductIdGroupCoverageId(glbPolicy.Product.Id, glbRisk.CoverageGroupId, glbPolicy.Prefix.Id);
                pressBtnAcept = true;
            }
        } else {
            RiskAircraftCoverage.GetInsuredObjectsByProductIdGroupCoverageId(glbPolicy.Product.Id, glbRisk.CoverageGroupId, glbPolicy.Prefix.Id);
    ...
    static LoadTemporal(policyData) {
        ...
        RiskAircraftCoverage.CalculateDays();
    }
    bindEvents() {
        ...
        $('#btnAcceptCoverage').on('click', RiskAircraftCoverage.AddCoverage);
        $('#btnSaveCoverage').on('click', RiskAircraftCoverage.SaveCoverages);
        ...
        $('#inputDeclaredAmount').focusout(function (event) {
            event.stopPropagation();
            $(this).val(FormatMoney($(this).val()));
            RiskAircraftCoverage.UpdateAmounts();
        });
        ...
        //    event.stopPropagation();
        //    RiskAircraftCoverage.QuotationCoverage();
        //});
    }
    static GetCoverageModel() {
        var coverageModel = $("#formAircraftCoverage").serializeObject();
        ...
        var InsuredObject = $("#formObjectAircraft").serializeObject();
        ...
        if (EditSubLineBusiness != null && EditSubLineBusiness != undefined) {
            ...
            if (CoveragesListAircraft != null && CoveragesListAircraft.length > 0) {
                for (var i = 0; i < CoveragesListAircraft.length; i++) {
                    if (CoveragesListAircraft[i].Id == coverageModel.CoverageId) {
                        coverageModel.SubLineBusiness = CoveragesListAircraft[i].SubLineBusiness;
                        break;
                    }
                }
            }
    ...
    static UpdatePremium() {
        ...
        $.each($("#listCoveragesEdit").UifListView('getData'), function (key, value) {
            ...
            //if (this.DeclaredAmount != undefined || this.PremiumAmount != undefined) {
    ...
    CloseCoverage() {
        RiskAircraftCoverage.ClearCoverage();
        IsEditing = false;
        router.run("prtRiskAircraft");
    }
    ...
    static ShowPanelsCoverage(Menu) {
        switch (Menu) {
            ...
            case MenuType.Concepts:
                $("#modalConcepts").UifModal('showLocal', AppResources.LabelConcepts);
                break;
        }
    }
    static QuotationCoverage() {
        var Coverage = RiskAircraftCoverage.GetCoverageModel();
        var selectedCoverage = $("#selectCoverage").UifSelect('getSelected');
        if (selectedCoverage != "" && selectedCoverage != null) {
            RiskAircraftCoverageRequest.QuotationCoverage(Coverage, aircraftDto, false, true).done(function (data) {
                if (data.success) {
                    if (data.result != null) {
            ...
            RiskAircraftCoverageRequest.QuotationCoverage(coverage, aircraftDto, false, true).done(function (data) {
                if (data.success) {
                    if (data.result != null) {
                        ...
                        var listCoveranges = $("#listCoveragesEdit").UifListView("getData");
                        $.each(listCoveranges, function (index, value) {
                            if (parseInt(value.DeclaredAmount) > 0 || parseInt(value.PremiumAmount) > 0) {
                                isCalculatePremiunCoverange = true;
                            }
                        });
                    }
                    ...
                    RiskAircraftCoverage.UpdatePremium();
                    RiskAircraftCoverage.ClearCoverage();
                }
                else {
                    $.UifNotify('show', { 'type': 'info', 'message': data.result, 'autoclose': true });
    ...
    newCoverage() {
        if (pressBtnAcept) {
            ...
            RiskAircraftCoverageRequest.GetCoveragesByInsuredObjectId(insuredObjectId, coverages).done(function (data) {
                if (data.success) {
                    CoveragesListAircraft = data.result;
                    RiskAircraftCoverage.ClearCoverage();
    ...
    static SaveInsuredObject() {
        ...
        RiskAircraftCoverageRequest.SaveInsuredObject(glbRisk.Id, insuredObject, glbPolicy.Id, glbRisk.CoverageGroupId).done(function (data) {
            if (data.success) {
            }
            else {
                $.UifNotify('show', { 'type': 'info', 'message': data.result, 'autoclose': true });
    ...
    ShowDetail() {
        $("#formObjectAircraft").validate();
        if ($("#formObjectAircraft").valid()) {
    ...
    static SaveCoverages(event) {
        ...
        RiskAircraftCoverageRequest.SaveCoverages(glbPolicy.Id, glbRisk.Id, $("#listCoveragesEdit").UifListView('getData'), insuredObjectId).done(function (data) {
            if (data.success) {
    ...
    static GetCalculationTypes() {
        RiskAircraftCoverageRequest.GetCalculationTypes().done(function (data) {
            if (data.success) {
    ...
    static GetRateTypes() {
        RiskAircraftCoverageRequest.GetRateTypes().done(function (data) {
            if (data.success) {
    ...
    static GetInsuredObjectsByProductIdGroupCoverageId(productId, groupCoverageId, prefixId) {
        if (IsEditing) {
            RiskAircraftCoverageRequest.GetInsuredObjectsByProductIdGroupCoverageId(productId, groupCoverageId, prefixId, aircraftDto.InsuredObjects).done(function (data) {
                if (data.success) {
                    ...
                    RiskAircraftCoverage.GetFormInsuredObject();
                }
                else {
                    $.UifNotify('show', { 'type': 'info', 'message': data.result, 'autoclose': true });
                }
            ...
            RiskAircraftCoverageRequest.GetInsuredObjectsByProductIdGroupCoverageId_inCoverages(productId, groupCoverageId, prefixId, aircraftDto.InsuredObjects).done(function (data) {
                if (data.success) {
                    $("#selectInsuredObject").UifSelect({ sourceData: data.result, selectedId: glbCoverage.CoverageId });
                    insuredObjectId = glbCoverage.CoverageId;
                    if (data.result.length == 1) {
    ...
    static GetCoveragesByRiskId(riskId, temporalId) {
        RiskAircraftCoverageRequest.GetCoveragesByRiskId(riskId, temporalId).done(function (data) {
            if (data.success) {
                RiskAircraftCoverage.LoadCoverages(data.result);
            }
            return data.success;
    ...
    static GetTemporalCoveragesByRiskIdInsuredObjectId(riskId, insuredObjectId) {
        RiskAircraftCoverageRequest.GetTemporalCoveragesByRiskIdInsuredObjectId(riskId, insuredObjectId).done(function (data) {
            if (data.success) {
    ...
    static GetCoveragesByObjectId(selectInsuredObjectId) {
        RiskAircraftCoverage.GetTemporalCoveragesByRiskIdInsuredObjectId(glbRisk.Id, parseInt(selectInsuredObjectId), true);
    }
    ...
    GetInsuredObjectId(event, itemSelected) {
        ...
        RiskAircraftCoverage.GetFormInsuredObject();
        RiskAircraftCoverage.GetCoveragesByObjectId(insuredObjectId);
    ...
    EditCoverage(event, data, index) {
        ...
        if (coverageData != null) {
            ...
            if (index != null) {
                RiskAircraftCoverage.SetCoverageEdit(data);
            }
            ...
            if (coverageData.DeductibleId != null) {
                RiskAircraftCoverage.GetDeductiblesByCoverageId(coverageData.Id, coverageData.DeductibleId);
            }
            else {
                RiskAircraftCoverage.GetDeductiblesByCoverageId(coverageData.Id, 0);
            }
        }
        else {
    ...
    CoverageDelete(event, data) {
        RiskAircraftCoverage.DeleteCoverage(data);
    }
    ...
    static GetDeductiblesByCoverageId(coverageId, selectedId) {
        RiskAircraftCoverageRequest.GetDeductiblesByCoverageId(coverageId).done(function (data) {
            if (data.success) {
                if (selectedId == 0) {
