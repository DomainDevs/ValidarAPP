...
var riskData = null;
class RiskMarine extends Uif2.Page {
    getInitialState() {
        riskController = 'RiskMarine';
        if (glbRisk == null) {
            glbRisk = { Id: 0, Object: "RiskMarine", formRisk: "#formMarine", RecordScript: false, Class: RiskMarine };
        }

        RiskMarine.GetUsesMarine(6);
        ...
        RiskMarine.ShowPanelsRisk(MenuType.Risk);
        RiskMarine.GetRisksByTemporalId(glbPolicy.Id, glbRisk.Id);
    }
        ...
        if (glbPersonOnline != null) {
            if (glbPersonOnline.Rol == 2) {
                if (glbPersonOnline.IndividualId > 0) {
                    RiskMarine.GetInsuredsByDescriptionInsuredSearchTypeCustomerType(glbPersonOnline.IndividualId, InsuredSearchType.IndividualId, glbPersonOnline.CustomerType);
                ...
                if (glbRisk.Id > 0) {
                    RiskMarine.GetRiskById(glbRisk.Id);
                }
                else {
                    RiskMarine.LoadViewModel(glbPersonOnline.ViewModel);
                }
    ...
    bindEvents() {
        ...
        $('#selectRisk').on('itemSelected', RiskMarine.ChangeRisk);
        ...
        $("#btnAddRisk").on('click', RiskMarine.ClearControls);
        $("#btnBeneficiary").on('click', { Menu: MenuType.Beneficiaries }, RiskMarine.ShowPanelsRisk);
        $("#btnClauses").on('click', { Menu: MenuType.Clauses }, RiskMarine.ShowPanelsRisk);
        $("#btnTexts").on('click', { Menu: 5 }, RiskMarine.ShowPanelsRisk);
        $("#btnDeleteRisk").on('click', RiskMarine.DeleteRisk);
    ...
    CoverageDelete(event, data) {
        RiskMarine.DeleteCoverage(data);
    }
    static DeleteCoverage(data) {
        if (data.IsMandatory == true) {
            ...
            RiskMarine.UpdatePremiunAndAmountInsured();
    ...
    EditCoverage(event, data, index) {
        ...
        if (data.IsVisible == false) {
            ...
            IsEditing = true;
            RiskMarine.SaveRisk(MenuType.Coverage, data.Id);
        }
    }

    RedirectClauses(event) {
        RiskMarine.SaveRisk(MenuType.Clauses, 0);
    }
    ...
    SearchInsured() {
        if ($("#inputInsured").val().trim().length > 0) {
            $("#inputInsured").data("Object", null);
            $("#inputInsured").data("Detail", null);
            RiskMarine.GetInsuredsByDescriptionInsuredSearchTypeCustomerType($("#inputInsured").val().trim(), InsuredSearchType.DocumentNumber, null);
        }
    }

    static GetUsesMarine(prefixId) {
        RiskMarineRequest.GetUsesMarine(prefixId).done(function (data) {
            if (data.success) {
                $("#selectMarineUseIds").UifSelect({ sourceData: data.result });
            }
            else {
        ...
        switch (Menu) {
            case MenuType.Risk:
                if (policyIsFloating != null && policyIsFloating != undefined) {
                    if (!policyIsFloating) {
                        $('#SpecificMarine').show();
                        $('#AutomaticMarine').hide();
                    } else {
                        $('#AutomaticMarine').show();
                        $('#SpecificMarine').hide();
                    }
                }
                RiskMarine.GetInsuredsByDescriptionInsuredSearchTypeCustomerType(glbPolicy.Holder.IndividualId, InsuredSearchType.IndividualId, glbPolicy.Holder.CustomerType);
                break;
            case MenuType.Texts:
                RiskMarine.GetTextsByRiskId(glbRisk.Id);
    ...
    static GetTextsByRiskId(riskId) {
        RiskMarineRequest.GetTextsByRiskId(riskId).done(function (data) {
            if (data.success) {
    ...
    static GetRisksByTemporalId(temporalId, selectedId) {
        RiskMarineRequest.GetCiaRiskByTemporalId(temporalId).done(function (data) {
            if (data.success) {
                if (data.result != null) {
                    if (selectedId > 0) {
                ...
                if (glbPersonOnline != null) {
                    ...
                    RiskMarine.GetRiskById($("#selectRisk option[Value!='']")[0].value);
                    ...
                    RiskMarine.SetInitialMarine();
                }
                RiskMarine.GetPolicyTypes(glbPolicy.Prefix.Id, glbPolicy.PolicyType.Id);
                RiskMarine.LoadTitle();
            }
            else {
                $.UifNotify('show', { 'type': 'info', 'message': data.result, 'autoclose': true });
            }
    ...
    static GetRiskById(id) {
        if (id != null && id != "") {
            RiskMarineRequest.GetRiskById(id).done(function (data) {
                if (data.success) {
                    if (data.result != null) {
                        glbRisk.RecordScript = true;
                        glbRisk.Id = id;
                        RiskMarine.LoadRisk(data.result);
                        RiskMarine.LoadCoverages(data.result.InsuredObjects, data.result.CoverageGroupId);
                        RiskMarine.UpdatePremiunAndAmountInsured();
                    }
                    else {
    ...
    static ChangeRisk(event, selectedItem) {
        if (selectedItem.Id > 0) {
            ...
            RiskMarine.GetRiskById(selectedItem.Id);
        }
        else {
            RiskMarine.ClearControls();
        }
    }

    static SetInitialMarine() {
        RiskMarine.GetGroupCoverages(glbPolicy.Product.Id, 0);
        RiskMarine.RunRules(glbPolicy.Product.CoveredRisk.PreRuleSetId);
    }
            ...
            UnderwritingRequest.GetInsuredsByDescriptionInsuredSearchTypeCustomerType(description, insuredSearchType, customerType, glbPolicy.TemporalType).done(function (data) {
                if (data.success) {
                    if (data.result.length == 0) {
                        ...
                        RiskMarine.LoadInsured(data.result[0]);
                    }
                    else if (data.result.length > 1) {
                        modalListType = 1;
                        ...
                        RiskMarine.ShowModalList(dataList);
    ...
    static LoadInsured(insured) {
        ...
        if (individualSearchType == 1) {
            ...
            if (insured.CustomerType == CustomerType.Individual) {
                $("#inputInsured").data("Detail", RiskMarine.GetIndividualDetails(insured.CompanyName));
            ...
            if (insured.CustomerType == CustomerType.Individual) {
                $("#inputBeneficiaryName").data("Detail", RiskMarine.GetIndividualDetails(insured.CompanyName));
                $("#btnConvertProspect").hide();
            }
            else if (glbPolicy.TemporalType != TemporalType.Quotation) {
                $("#btnConvertProspect").show();
    ...
    static RunRules(ruleSetId) {
        RiskMarineRequest.RunRules(glbPolicy.Id, ruleSetId).done(function (data) {
            if (data.success) {
                RiskMarine.LoadRisk(data.result);
            }
            else {
    ...
    static LoadRisk(riskData) {
        RiskMarine.GetGroupCoverages(glbPolicy.Product.Id, riskData.CoverageGroupId);
        $("#selectGroupCoverage").UifSelect("setSelected", riskData.CoverageGroupId);
        $("#selectMarineUseIds").UifSelect("setSelected", riskData.UseId);
        $("#inputCurrentManufacturing").val(riskData.YearManufacturing);
        $("#inputNameBoat").val(riskData.BoatName);
    ...
    AddCoverage(event) {
        IsEditing = false;
        RiskMarine.SaveRisk(MenuType.Coverage, 0);
    }
    ...
    static SaveRisk(redirec, coverageId) {
        ...
        $("#formMarine").validate();
        if ($("#formMarine").valid()) {
            if (!RiskMarine.ValidateBoatName()) {
                $.UifNotify('show', { 'type': 'danger', 'message': AppResources.InvalidBoatName, 'autoclose': true });
                return;
            }

            var riskData = RiskMarine.GetRiskDataModel();
            marineDto = riskData;
            RiskMarineRequest.SaveRisk(glbPolicy.Id, riskData, dynamicProperties).done(function (data) {
                if (data.success) {
                    if (data.result != null) {
                        RiskMarine.UpdateGlbRisk(data.result);
                        RiskMarine.ShowSaveRisk(glbRisk.Id, redirec, coverageId);
                    }
                    else {
    ...
    static ShowSaveRisk(riskId, redirec, coverageId) {
        if (redirec != 9) {
            if ($("#selectRisk").UifSelect("getSelected") == null || $("#selectRisk").UifSelect("getSelected") == 0 || $("#selectRisk").UifSelect("getSelected") == "") {
                RiskMarine.GetRisksByTemporalId(glbPolicy.Id, riskId);
            }
        }
        var events = null;
        ...
        if (events !== TypeAuthorizationPolicies.Restrictive) {
            RiskMarine.RedirectAction(redirec, riskId, coverageId);
        }
    ...
    static RedirectAction(redirec, riskId, coverageId) {
        switch (redirec) {
            case MenuType.Underwriting:
                RiskMarine.ReturnUnderwriting();
                break;
            case MenuType.Risk:
                $.UifNotify('show', { 'type': 'info', 'message': AppResources.MessageSavedRiskSuccessfully, 'autoclose': true });
                break;
            case MenuType.Coverage:
                RiskMarine.ReturnCoverage(coverageId);
                break;
            case MenuType.Beneficiaries:
                RiskMarine.ShowPanelsRisk(MenuType.Beneficiaries);
                break;
            case MenuType.AdditionalData:
                RiskMarine.ShowPanelsRisk(MenuType.AdditionalData);
                break;
            case MenuType.Texts:
                RiskMarine.ShowPanelsRisk(MenuType.Texts);
                break;
            case MenuType.Clauses:
                RiskMarine.ShowPanelsRisk(MenuType.Clauses);
                break;
            case MenuType.Concepts:
                RiskMarine.ShowPanelsRisk(MenuType.Concepts);
                break;
            case MenuType.Script:
                RiskMarine.ShowPanelsRisk(MenuType.Script);
                break;
            default:
                $.UifNotify('show', { 'type': 'danger', 'message': AppResources.ErrorSaveRisk, 'autoclose': true });
                break;
    ...
    static ReturnCoverage(coverageId) {
        glbCoverage = {
            ...
            Class: RiskMarineCoverage
        }
        router.run("prtRiskMarineCoverage");
    }

    static GetPolicyTypes(prefixId, id) {
        RiskMarineRequest.GetPolicyType(prefixId, id).done(function (data) {
            if (data.success) {
    ...
    ChangeGroupCoverage(event, selectedItem) {
        if (selectedItem.Id > 0) {
            RiskMarine.GetInsuredObjectsByProductIdGroupCoverageId(selectedItem.Id);
        }
        else {
    ...
    static GetInsuredObjectsByProductIdGroupCoverageId(groupCoverageId) {
        RiskMarineRequest.GetInsuredObjectsByProductIdGroupCoverageId(glbPolicy.Product.Id, groupCoverageId, glbPolicy.Prefix.Id).done(function (data) {
            if (data.success) {
                $("#listCoverages").UifListView({ source: null, customDelete: true, customAdd: true, customEdit: true, add: true, edit: true, delete: true, displayTemplate: "#coverageTemplate", title: AppResources.LabelInsuredObjects, height: 400 });
                RiskMarine.LoadCoverages(data.result, groupCoverageId);
            }
            else {
                $.UifNotify('show', { 'type': 'info', 'message': data.result, 'autoclose': true });
            }
    ...
    static GetGroupCoverages(productId, selectedId) {
        RiskMarineRequest.GetGroupCoverages(productId).done(function (data) {
            if (data.success) {
    ...
    static GetRiskDataModel() {
        var riskData = $("#formMarine").serializeObject();
        ...
        riskData.UseId = $('#selectMarineUseIds').UifSelect('getSelected');
        riskData.CoverageGroupId = $('#selectGroupCoverage').UifSelect('getSelected');
        riskData.PolicyId = glbPolicy.Id;
        riskData.IsFloating = glbPolicy.PolicyType.IsFloating;
        riskData.UseDescription = $($('#selectMarineUseIds option:selected')[0]).text();
        riskData.Description = $($('#selectMarineUseIds option:selected')[0]).text() + ' - ' + $("#inputNameBoat").val();
        ...
        //riskData.DocumentExpedition = new Date(parseInt(Expiration));
    ...
    static UpdateGlbRisk(data) {
        ...
        glbRisk.Object = "RiskMarine";
        glbRisk.formRisk = "#formMarine";
        glbRisk.RecordScript = recordScript;
        glbRisk.Class = RiskMarine;
    }
    ...
    static ReturnUnderwriting() {
        ...
        if (glbPolicy.Endorsement.EndorsementType == EndorsementType.Modification) {
            glbPolicy.EndorsementController = "MarineModification";
    ...
    static GetPremium() {
        if ($('#inputInsured').data("Object") != null) {
            ...
            if ($("#formMarine").valid()) {
                ...
                if (recordScript) {
                    var riskData = RiskMarine.GetRiskDataModel();
                    RiskMarineRequest.GetPremium(glbPolicy.Id, riskData, dynamicProperties).done(function (data) {
                        if (data.success) {
                            RiskTrasnport.LoadRisk(data.result);
                        }
                        else {
                            $("#inputPremium").text(0);
                            $("#inputAmountInsured").text(0);
    ...
    static DeleteRisk() {
        if ($("#selectRisk").UifSelect("getSelected") > 0) {
            RiskMarineRequest.DeleteRisk(glbPolicy.Id, $("#selectRisk").UifSelect("getSelected")).done(function (data) {
                if (data.success) {
                    if (parseInt(glbPolicy.Endorsement.EndorsementType, 10) == EndorsementType.Emission || parseInt(glbPolicy.Endorsement.EndorsementType, 10) == EndorsementType.Renewal || glbRisk.Status == RiskStatus.Included) {
                        RiskMarine.ClearControls();
                        RiskMarine.GetRisksByTemporalId(glbPolicy.Id);
    ...
    static ClearControls() {
        ...
        $("#selectMarineUseIds").UifSelect("setSelected", null);
        $('#selectedConceptsRisk').text("");

    }

    Accept() {
        RiskMarine.SaveRisk(MenuType.Risk, 0);
        ScrollTop();
        marineDto = null;
    }

    Close() {
        glbRisk = { Id: 0, Object: "RiskMarine", Class: RiskMarine };
        RiskMarine.UpdatePolicyComponents();
        marineDto = null;
    }
    ...
    static ValidateBoatName() {
        var regexNumberRegister = /^[A-záéíóúÁÉÍÓÚ \-\_]{1,50}$/;
        return regexNumberRegister.test($('#inputNameBoat').val());
    }
}