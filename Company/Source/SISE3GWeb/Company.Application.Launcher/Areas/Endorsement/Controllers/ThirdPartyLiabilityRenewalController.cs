...
{
    public class ThirdPartyLiabilityRenewalController : RenewalController
    {
        ...
        public ActionResult CreateTemporal(RenewalViewModel renewalModel)
        {
            try
            {
                ...
                if (DelegateService.utilitiesServiceCore.GetEndorsementControlById(id, UserId))
                {
                    ...
                    var policy = DelegateService.thirdPartyLiabilityRenewalServiceCia.CreateRenewal(CompanyPolicy);
                    if (policy != null)
                    {
