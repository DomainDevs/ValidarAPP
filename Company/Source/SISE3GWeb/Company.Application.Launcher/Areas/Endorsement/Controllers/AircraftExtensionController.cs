...
{
    public class AircraftExtensionController : ExtensionController
    {
        public ActionResult CreateTemporal(ExtensionViewModel extensionModel)
        {
            try
            {
                ...
                {
                    var policyDTO = ModelAssembler.CreateCompanyEndorsement(extensionModel);

                    var policy = DelegateService.aircraftExtensionService.CreateExtension(policyDTO);
