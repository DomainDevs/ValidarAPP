...
////////////////////////////////////////////////////////////////////////////
//    VALIDA LOS PROCESOS PARA REFRESCAR GRILLA DE REPORTES GENERADOS     //
///////////////////////////////////////////////////////////////////////////

function validateProcessReport(processTable, top) {

    var urlFileName;

    for (var i in processTable) {

        var progresNum = processTable[i].Progress.split(" ");
        urlFileName = processTable[i].UrlFile;

        if (parseInt(progresNum[0]) < 100 || urlFileName == Resources.GenerateDocument) {
            return false;
        }
        else {
            if (parseInt(i) == top - 1) {
...
// Verifica a nivel de controlador existencia de archivo generado x reportes
function reportFileExist(url_path, alertForm, focusControl) {
    $.ajax({
        async: false,
        type: "POST",
        url: ACC_ROOT + "Reports/ValidateReportFile",
        data: JSON.stringify({ "urlReport": url_path }),
        dataType: "json",
        ...
        if (data.success) {
            ...
            $("#" + alertForm).show();
            $("#" + alertForm).UifAlert('show', Resources.WarningNotFileExist, "warning");
            $("#" + focusControl).focus();
        }
    });
}
...
function RemoveFormatMoney(inpText, numerofDecimalPlaces) {
    if (typeof inpText !== 'string') {
        return inpText;
    }
    ...
    var isNegative = (new RegExp('^\\((.*)\\)$')).test(inpText);
    if (isNegative === true) {
        inpText = inpText.replace('(', '').replace(')', '');
    }
    if (arguments.length < 2) {
        numerofDecimalPlaces = numberDecimal;
    }

    if ((new RegExp('^(\\d){1,3}(\\.(\\d){3})*(,(\\d){0,' + numerofDecimalPlaces + '})?$')).test(inpText)) {
        inpText = inpText.replace(new RegExp('\\.', 'g'), '').replace(',', '.');
    }
    else if ((new RegExp('^(\\d){1,3}(,(\\d){3})*(\\.(\\d){0,' + numerofDecimalPlaces + '})?$')).test(inpText)) {
        inpText = inpText.replace(new RegExp(',', 'g'), '');
    }
    else if ((new RegExp('^(\\d+)\\.(\\d){0,' + numerofDecimalPlaces + '}$')).test(inpText)) {
        inpText = inpText.padEnd(inpText.indexOf('.') + (numerofDecimalPlaces + 1), '0');
    }
    else if ((new RegExp('^(\\d+),(\\d){0,' + numerofDecimalPlaces + '}$')).test(inpText)) {
        inpText = inpText.padEnd(inpText.indexOf(',') + (numerofDecimalPlaces + 1), '0').replace(',', '.');
    }
    else if ((new RegExp('^(\\d+),(\\d){0,}$')).test(inpText)) {
        inpText = inpText.padEnd(inpText.indexOf(',') + (numerofDecimalPlaces + 1), '0').substr(0, inpText.indexOf(',') + numerofDecimalPlaces + 1).replace(',', '.');
    }
    else if ((new RegExp('^(\\d+)\\.(\\d){0,}$')).test(inpText)) {
        inpText = inpText.padEnd(inpText.indexOf('.') + (numerofDecimalPlaces + 1), '0').substr(0, inpText.indexOf('.') + numerofDecimalPlaces + 1);
    }

    if (isNegative === true) {
        inpText = '-' + inpText;
    }
    var result = parseFloat(inpText);

    return (isNaN(result) ? 0 : result);
}

function FormatMoneySymbol(val, numberofDecimals) {
    if (typeof val === 'string' && val.startsWith(currencySymbol)) {
        return val;
    }
    return currencySymbol + ' ' + FormatMoneyNoSymbol(val, numberofDecimals);
}

function FormatMoneyNoSymbol(val, numberofDecimals) {
    ...
    let txt = FormatMoney(val, numberofDecimals);
    if (parseFloat(val) < 0) {
        if (txt.startsWith('-')) {
            txt = txt.replace('-', '');
        }
...
function fix(val) {
    var _base = Math.pow(10, numberDecimal);
    return Math.round((val + Number.EPSILON) * _base) / _base;
}
    ...
    for (var k in obj) {
        if (Array.isArray(obj[k]) && obj[k].length > 0) {
            for (var i = 0; i < obj[k].length; i++) {
                obj[k][i] = prepareRequest(obj[k][i]);
            }
        }
        else if (typeof obj[k] === 'object') {
            obj[k] = prepareRequest(obj[k]);
        }
...
function isEmptyArray(obj) {
    return isEmpty(obj) || (Array.isArray(obj) && obj.length == 0);
}

function isEmptyorZero(text) {
    return isEmpty(text) || text == "0";
}
...
function isMoneyEmpty(obj) {
    return isEmpty(obj) || RemoveFormatMoney(obj) == 0;
}
