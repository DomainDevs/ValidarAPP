...
namespace Sistran.Company.Application.Marines.MarineBusinessService.EEProvider.Assemblers
{
    public class EntityAssembler
    {
        ...
        /// <summary>
        /// Crea la fachada de los riesgos de Marinee
        /// </summary>
        /// <param name="facade"></param>
        /// <param name="companyMarine"></param>
        internal static void CreateFacadeRiskMarine(Facade facade, CompanyMarine companyMarine)
        {
            facade.SetConcept(CompanyRuleConceptRisk.TempId, companyMarine.Risk.Policy.Endorsement.TemporalId);
            facade.SetConcept(CompanyRuleConceptRisk.RiskId, companyMarine.Risk.RiskId);
            facade.SetConcept(CompanyRuleConceptRisk.HolderTypeCode, 2);
            facade.SetConcept(CompanyRuleConceptRisk.EndorsementId, companyMarine.Risk.Policy.Endorsement.Id);
            facade.SetConcept(CompanyRuleConceptRisk.PolicyTypeViewCode, companyMarine.Risk.Policy.PolicyType.Id);
            facade.SetConcept(CompanyRuleConceptRisk.HolderTypeCode, companyMarine.Risk.Policy.Holder.CustomerType);
            facade.SetConcept(CompanyRuleConceptRisk.CustomsAgentId, companyMarine.Risk.Policy.Agencies[0].Agent.IndividualId);
            facade.SetConcept(CompanyRuleConceptRisk.BeneficiariesCount, companyMarine.Risk.Beneficiaries?.Count == null ? 0 : companyMarine.Risk.Beneficiaries?.Count);
            facade.SetConcept(CompanyRuleConceptRisk.CoveredRiskTypeCode, (int)companyMarine.Risk.Policy.Product.CoveredRisk.CoveredRiskType);
            facade.SetConcept(CompanyRuleConceptRisk.RiskStatusCode, companyMarine.Risk.Status == null ? (int?)null : (int)companyMarine.Risk.Status);
            facade.SetConcept(CompanyRuleConceptRisk.RiskOriginalStatusCode, companyMarine.Risk.OriginalStatus == null ? (int?)null : (int)companyMarine.Risk.OriginalStatus);
            facade.SetConcept(CompanyRuleConceptRisk.ConditionText, companyMarine.Risk.Text == null ? string.Empty : companyMarine.Risk.Text.TextBody);
            facade.SetConcept(CompanyRuleConceptRisk.RatingZoneCode, companyMarine.Risk.RatingZone?.Id == 0 ? null : companyMarine.Risk.RatingZone?.Id);
            facade.SetConcept(CompanyRuleConceptRisk.CoverageGroupId, companyMarine.Risk.GroupCoverage?.Id == 0 ? null : companyMarine.Risk.GroupCoverage?.Id);
            facade.SetConcept(CompanyRuleConceptRisk.OperationId, companyMarine.Risk.Id);
            facade.SetConcept(CompanyRuleConceptRisk.LimitsRcCode, companyMarine.Risk.LimitRc?.Id == 0 ? null : companyMarine.Risk.LimitRc?.Id);
            facade.SetConcept(CompanyRuleConceptRisk.LimitsRcSum, companyMarine.Risk.LimitRc?.LimitSum == 0 ? null : companyMarine.Risk.LimitRc?.LimitSum);
            facade.SetConcept(CompanyRuleConceptRisk.PremiunRisk, companyMarine.Risk.Premium);
            facade.SetConcept(CompanyRuleConceptRisk.AmountInsured, companyMarine.Risk?.AmountInsured ?? 0);
            facade.SetConcept(CompanyRuleConceptRisk.ConditionTextId, companyMarine.Risk.Text?.Id == 0 ? null : companyMarine.Risk.Text?.Id);
            facade.SetConcept(CompanyRuleConceptRisk.ConditionText, companyMarine.Risk.Text?.TextBody);

            if (companyMarine.Risk.MainInsured != null)
            {
                facade.SetConcept(CompanyRuleConceptRisk.InsuredId, companyMarine.Risk.MainInsured.IndividualId == 0 ? (int?)null : companyMarine.Risk.MainInsured.IndividualId);
                facade.SetConcept(CompanyRuleConceptRisk.CustomerTypeCode, (int)companyMarine.Risk.MainInsured.CustomerType);
                facade.SetConcept(CompanyRuleConceptRisk.IsInsuredPayer, companyMarine.Risk.MainInsured.IndividualId == companyMarine.Risk.Policy.Holder.IndividualId);
                if (companyMarine.Risk.MainInsured.IdentificationDocument != null)
                {
                    facade.SetConcept(CompanyRuleConceptRisk.InsuredIdentificationDocument, companyMarine.Risk.MainInsured.IdentificationDocument.Number);
                }
                if (companyMarine.Risk.MainInsured.IndividualType == IndividualType.Person)
                {
                    int insuredAge = 0;
                    if (companyMarine.Risk.MainInsured.BirthDate.GetValueOrDefault() > DateTime.MinValue)
                    {
                        insuredAge = (DateTime.Today - companyMarine.Risk.MainInsured.BirthDate.Value).Days / 365;
                    }

                    facade.SetConcept(CompanyRuleConceptRisk.InsuredAge, insuredAge);
                    facade.SetConcept(CompanyRuleConceptRisk.InsuredGender, companyMarine.Risk.MainInsured.Gender);
                    facade.SetConcept(CompanyRuleConceptRisk.InsuredBirthDate, companyMarine.Risk.MainInsured.BirthDate);
                }
            }
            if (companyMarine.Risk.Beneficiaries != null && companyMarine.Risk.Beneficiaries.Count > 0)
            {
                CompanyBeneficiary beneficiary = companyMarine.Risk.Beneficiaries.First();
                if (beneficiary != null)
                {
            ...
            if (companyMarine.Risk?.CompanyClaimsBills != null)
            {
                var claimsBills = companyMarine.Risk.CompanyClaimsBills;
            }
            if (companyMarine.Risk?.DynamicProperties != null)
            {
                foreach (DynamicConcept dynamicConcept in companyMarine.Risk.DynamicProperties)
                {
