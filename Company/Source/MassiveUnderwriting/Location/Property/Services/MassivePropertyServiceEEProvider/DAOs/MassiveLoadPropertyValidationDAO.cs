...
{
    public class MassiveLoadPropertyValidationDAO
    {
        public List<Validation> GetValidationsByFiles(List<File> files, MassiveEmission massiveEmission, int agentId, int agentTypeId, int productRequestId, int requestId, int billingGroupId)
        {
            ...
            foreach (File file in files)
            {
                ...
                int request = 0;
                int prefixId = massiveEmission.Prefix.Id;
                ...
                foreach (Template template in file.Templates)
                {
                    ...
                    if (template.PropertyName == TemplatePropertyName.EmissionProperty)
                    {
                        ...
                        request = (int)DelegateService.commonService.GetValueByField<int>(row.Fields.Find(p => p.PropertyName == FieldPropertyName.RequestGroup));
                    ...
                    {
                        ValidationsByInsuredObjects(template);
                        validationInsuredObjects = GetValidationsInsuredObjectPeriodAndRate(file.Id, template);
                    ...
                    if (!template.Rows.Any(r => r.HasError))
                    {
                        if (template.PropertyName == TemplatePropertyName.EmissionProperty)
                        {
                            validations.AddRange(DelegateService.massiveUnderwritingService.GetGeneralValidationsByEmissionTemplate(file.Id, massiveEmission.Prefix.Id, template, row, agentId, agentTypeId, productRequestId, requestId, billingGroupId));
                            validations.AddRange(GetSpecificValidationsByEmissionTemplate(file, template, row, massiveEmission, withRequestGrouping));
                            ...
                            if (!string.IsNullOrEmpty(row.Fields.Find(y => y.PropertyName == FieldPropertyName.RiskLongitude).Value))
                            {
                                ValidationRegularExpression validation = new ValidationRegularExpression();
                                validation.Id = file.Id;
                                validation.FieldDescription = row.Fields.Find(x => x.PropertyName == FieldPropertyName.RiskLongitude).Description;
                                validation.ValidationId = (int)RegularExpression.longitude;
                                validation.ParameterValue = (string)DelegateService.commonService.GetValueByField<string>(row.Fields.Find(x => x.PropertyName == FieldPropertyName.RiskLongitude));
                                validationRegularExpressions.Add(validation);
                            }
                            if (!string.IsNullOrEmpty(row.Fields.Find(y => y.PropertyName == FieldPropertyName.RiskLatitude).Value))
                            {
                                ValidationRegularExpression validation = new ValidationRegularExpression();
                                validation.Id = file.Id;
                                validation.FieldDescription = row.Fields.Find(x => x.PropertyName == FieldPropertyName.RiskLatitude).Description;
                                validation.ValidationId = (int)RegularExpression.latitude;
                                validation.ParameterValue = (string)DelegateService.commonService.GetValueByField<string>(row.Fields.Find(x => x.PropertyName == FieldPropertyName.RiskLatitude));
                                validationRegularExpressions.Add(validation);
                            }
                        ...
                        if (template.PropertyName == TemplatePropertyName.InsuredObjects)
                        {
                            validations.AddRange(GetValidationsByInsuredObjectsTemplate(file, template, massiveEmission, productId, withRequestGrouping, groupCoverageId, request));
                        }

                        if (template.PropertyName == TemplatePropertyName.ModifyCoverages)
                        {
                            validations.AddRange(GetValidationsByModifyCoveragesTemplate(file, template, massiveEmission, productId, withRequestGrouping, groupCoverageId));
                        }

                        if (template.PropertyName == TemplatePropertyName.AdditionalIntermediaries)
                        {
                            validations.AddRange(GetValidationsByAdditionalIntermediariesTemplate(file, template, massiveEmission, productId, withRequestGrouping, prefixId));
                        }
                        ...
                        {
                            validations.AddRange(DelegateService.massiveUnderwritingService.GetValidationsClauseLevel(file.Id, prefixId, template, (int)CoveredRiskType.Location));
                        }
                    }
                }
            }
        ...
        private List<ValidationIdentificator> GetSpecificValidationsByEmissionTemplate(File file, Template template, Row row, MassiveEmission massiveEmission, bool withRequestGrouping)
        {
            ...
            if (!string.IsNullOrEmpty(row.Fields.Find(y => y.PropertyName == CompanyFieldPropertyName.RiskLevelZone).Value))
            {
                ...
                validation.ParameterValue1 = (int)DelegateService.commonService.GetValueByField<int>(row.Fields.Find(x => x.PropertyName == CompanyFieldPropertyName.RiskLevelZone));
                validations.Add(validation);
            }
        ...
        private List<ValidationIdentificator> GetValidationsByInsuredObjectsTemplate(File file, Template template, MassiveEmission massiveEmission, int productId, bool withRequestGrouping, int groupCoverageId, int request)
        {
            List<ValidationIdentificator> validations = new List<ValidationIdentificator>();
            ValidationIdentificator validation = new ValidationIdentificator();
            ...
            foreach (Row row in template.Rows)
            {
                if (!string.IsNullOrEmpty(row.Fields.Find(y => y.PropertyName == FieldPropertyName.InsuredObjectCode).Value))
                {
                    ...
                    {
                        validation.ParameterValue4 = request;
                    }
                    validations.Add(validation);
                }
        ...
        private List<ValidationIdentificator> GetValidationsByModifyCoveragesTemplate(File file, Template template, MassiveEmission massiveEmission, int productId, bool withRequestGrouping, int groupCoverageId)
        {
            List<ValidationIdentificator> validations = new List<ValidationIdentificator>();
            ValidationIdentificator validation = new ValidationIdentificator();
            int countAdditional = 1;
        ...
        private List<ValidationIdentificator> GetValidationsByAdditionalIntermediariesTemplate(File file, Template template, MassiveEmission massiveEmission, int productId, bool withRequestGrouping, int prefixId)
        {
            List<ValidationIdentificator> validations = new List<ValidationIdentificator>();
            ValidationIdentificator validation = new ValidationIdentificator();
            int countAdditional = 1;
        ...
        /// <summary>
        /// Validación Obligatoriedad Suma Asegurada
        /// </summary>
        /// <param name="row"></param>
        private void ValidationsByInsuredObjects(Template insuredObjectTemplate)
        {
            foreach (Row row in insuredObjectTemplate.Rows)
            {
                ...
                if (String.IsNullOrEmpty(row.Fields.FirstOrDefault(x => x.PropertyName == FieldPropertyName.InsuredObjectSumAssured).Value))
                {
                    row.HasError = true;
                    row.ErrorDescription += Errors.TheSumInsuredColumnOfTheInsuranceObjectsTemplateIsRequired + " " + insuredObjectId + KeySettings.ReportErrorSeparatorMessage();
                }
                else
                {
                    ...
                    {
                        row.HasError = true;
                        row.ErrorDescription += Errors.TheSumInsuredOfTheInsuranceObjectMustBeGreaterThanZeroObject + " " + insuredObjectId + KeySettings.ReportErrorSeparatorMessage();
                    }
                }
            }
        }
