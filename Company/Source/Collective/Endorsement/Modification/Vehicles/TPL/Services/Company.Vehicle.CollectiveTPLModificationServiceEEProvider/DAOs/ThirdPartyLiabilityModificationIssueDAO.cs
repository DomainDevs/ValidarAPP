...
namespace Sistran.Company.Application.Vehicles.CollectiveTPLModificationServiceEEProvider.DAOs
{
    ...
    public class ThirdPartyLiabilityModificationIssueDAO
    {
            ...
            try
            {
                ...
                if (emissionRows.Any(x => x.HasEvents == true))
                {
                    ...
                    ParallelHelper.ForEach(emissionRows.Where(x => x.HasEvents == false).ToList(), emissionRow =>
                    {
                        ...
                        {
                            pendingOperationRisk = DelegateService.utilitiesService.GetPendingOperationById(emissionRow.Risk.RiskId);
                        }
                        else
                        {
        ...
        private void CreateRisk(CollectiveEmissionRow emissionRow, List<PendingOperation> pendingOperations, CompanyPolicy companyPolicy)
        {
            try
            {
                ...
                var companyTplRisk = JsonConvert.DeserializeObject<CompanyTplRisk>(riskPendingOperation.Operation);
                companyTplRisk.Risk.Policy = companyPolicy;
                DelegateService.tplService.CreateRisk(companyTplRisk);
            }
            catch (Exception ex)
            {
                emissionRow.HasError = true;
