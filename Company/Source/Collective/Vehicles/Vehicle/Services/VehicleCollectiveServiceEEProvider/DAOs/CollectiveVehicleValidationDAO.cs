...
{
    public class CollectiveVehicleValidationDAO
    {
        public List<Validation> GetValidationsByFiles(List<File> files, CollectiveEmission collectiveEmission, int productId, int policyType, DateTime currentTo, DateTime currentFrom)
        {
                    ...
                    foreach (Template template in file.Templates)
                    {
                        if (template.PropertyName == TemplatePropertyName.RiskDetail)
                        {
                            ...
                            foreach (Row row in template.Rows)
                            {
                                ...
                                if (!string.IsNullOrEmpty(row.Fields.Find(y => y.PropertyName == FieldPropertyName.RiskLicensePlate).Value))
                                {
                        ...
                        else if (template.PropertyName == TemplatePropertyName.Accesories)
                        {
                            validations.AddRange(GetValidationsByAccesoriesTemplate(file, template, collectiveEmission, productId));
                        }
        ...
        private List<ValidationIdentificator> GetValidationsByAccesoriesTemplate(File file, Template template, CollectiveEmission collectiveEmission, int productId)
        {
            List<ValidationIdentificator> validations = new List<ValidationIdentificator>();
            int countAdditional = 1;
