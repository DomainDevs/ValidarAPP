...
namespace Sistran.Company.Application.Vehicles.VehicleCollectiveServices.EEProvider.DAOs
{
    ...
    public class CollectiveLoadProcessVehicleDAO
    {
        ...
        private void QuotateCollectiveEmissionRows(CollectiveEmission collectiveEmission, List<CollectiveEmissionRow> collectiveEmissionRows, CompanyPolicy companyPolicy)
        {
            ...
            {
                QuatateCollectiveEmissionRow(collectiveEmission, collectiveEmissionRow, companyPolicy, counter, hierarchy, ruleToValidateRisk, ruleToValidateCoverage);
                counter++;
            }
        }
        ...
        private void QuatateCollectiveEmissionRow(CollectiveEmission collectiveEmission, CollectiveEmissionRow collectiveEmissionRow, CompanyPolicy companyPolicy, int riskNum, int hierarchy, List<int> ruleToValidateRisk, List<int> ruleToValidateCoverage)
        {
            ...
            try
            {
                ...
                if (riskPendingOperation != null)
                {
                    ...
                    if (companyRisk != null)
                    {
                        ...
                        companyRisk.ActualDateMovement = DateTime.Now;
                        ...
                        if (companyRisk.Risk.Premium > 0)
                        {
                            ...
                            companyRisk.Risk.Number = riskNum;
        ...
        private void ExecuteCreatePolicy(List<CollectiveEmissionRow> emissionRows, CollectiveEmission collectiveEmission)
        {
            ...
            collectiveEmission.Status = MassiveLoadStatus.Issuing;
            DelegateService.collectiveService.UpdatCollectiveEmission(collectiveEmission);
            serializeCompanyPolicy = COMUT.JsonHelper.SerializeObjectToJson(companyPolicy);
            emissionRows = emissionRows.Where(x => x.HasEvents == false).ToList().OrderBy(x => x.RowNumber).ToList();

            foreach (CollectiveEmissionRow emissionRow in emissionRows)
            {
                try
                {
                    ...
                    string issuanceRiskJson = string.Format("{0}{1}{2}{3}{4}{5}{6}", serializeCompanyPolicy, (char)007, COMUT.JsonHelper.SerializeObjectToJson(emissionRow), (char)007, nameof(CompanyVehicle), (char)007, nameof(CollectiveEmissionRow));
