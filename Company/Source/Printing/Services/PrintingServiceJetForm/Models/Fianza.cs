...
namespace Sistran.Company.Application.PrintingServices.Models
{
    class Fianza : Reportes
    {
        ...
        protected override void setPolicyData(DataSet risk)
        {
            ...
            foreach (DataRow row in risk.Tables[1].Rows)
            {
                coveragePremiumValue += ReportServiceHelper.validateValue(row["COVERAGE_PREMIUM"].ToString(), new CultureInfo("en-US"));
            }
