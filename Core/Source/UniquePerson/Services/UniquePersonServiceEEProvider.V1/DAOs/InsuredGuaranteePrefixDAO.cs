...
namespace Sistran.Core.Application.UniquePersonService.V1.DAOs
{
    ...
    public class InsuredGuaranteePrefixDAO
    {
        ...
        public List<Models.InsuredGuaranteePrefix> GetInsuredGuaranteePrefix(int individualId, int guaranteeId)
        {
            ...
            filter.Property(UniquePersonV1.Entities.InsuredGuaranteePrefix.Properties.IndividualId, typeof(UniquePersonV1.Entities.InsuredGuaranteePrefix).Name);
            filter.Equal();
            filter.Constant(individualId);
            filter.And();
            filter.Property(UniquePersonV1.Entities.InsuredGuaranteePrefix.Properties.GuaranteeId, typeof(UniquePersonV1.Entities.InsuredGuaranteePrefix).Name);
            filter.Equal();
            filter.Constant(guaranteeId);

            BusinessCollection businessCollection = new BusinessCollection(DataFacadeManager.Instance.GetDataFacade().SelectObjects(typeof(UniquePersonV1.Entities.InsuredGuaranteePrefix), filter.GetPredicate()));

            stopWatch.Stop();
            Debug.WriteLine(stopWatch.Elapsed.ToString(), "Sistran.Core.Application.UniquePersonService.V1.DAOs.GetInsuredGuaranteePrefix");
        ...
        public void SaveGuaranteePrefix(List<Models.InsuredGuaranteePrefix> listPrefix, int guaranteeId)
        {
            ...
            {
                ObjectCriteriaBuilder filter = new ObjectCriteriaBuilder();
                filter.Property(UniquePersonV1.Entities.InsuredGuaranteePrefix.Properties.IndividualId, typeof(UniquePersonV1.Entities.InsuredGuarantee.Properties).Name);
                filter.Equal();
                filter.Constant(listPrefix.FirstOrDefault().IndividualId);
                filter.And();
                filter.Property(UniquePersonV1.Entities.InsuredGuaranteePrefix.Properties.GuaranteeId, typeof(UniquePersonV1.Entities.InsuredGuarantee.Properties).Name);
                filter.Equal();
                filter.Constant(guaranteeId);
                BusinessCollection businessCollection = new BusinessCollection(DataFacadeManager.Instance.GetDataFacade().SelectObjects(typeof(UniquePersonV1.Entities.InsuredGuaranteePrefix), filter.GetPredicate()));
                ...
                foreach (Models.InsuredGuaranteePrefix prefix in listPrefix)
                {
                    ...
                    UniquePersonV1.Entities.InsuredGuaranteePrefix insuredGuaranteePrefixEntity = EntityAssembler.CreateInsuredGuaranteePrefix(prefix);
