...
using PERMOD = Sistran.Core.Application.UniquePersonService.Models;
...
namespace Sistran.Core.Application.UniquePersonService.DAOs
{
    public class EconomicActivityDAO
    {
        ...
        public PERMOD.EconomicActivity GetEconomicActivitiesByEconomicActiviti(int EconomicActiviti)
        {
            ...
            businessCollection = new BusinessCollection(DataFacadeManager.Instance.GetDataFacade().SelectObjects(typeof(COMMEN.EconomicActivity), filter.GetPredicate()));
            stopWatch.Stop();
