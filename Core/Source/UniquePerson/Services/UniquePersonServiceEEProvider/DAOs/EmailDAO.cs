...
namespace Sistran.Core.Application.UniquePersonService.DAOs
{
    ...
    public class EmailDAO
    {
        ...
        public List<models.EmailType> GetEmailTypes()
        {
            ...
            stopWatch.Stop();
            Debug.WriteLine(stopWatch.Elapsed.ToString(), "Sistran.Core.Application.UniquePersonService.DAOs.GetEmailTypes");
        ...
        public List<models.Email> GetEmailsByIndividualId(int individualId)
        {
            ...
            businessCollection = new BusinessCollection(DataFacadeManager.Instance.GetDataFacade().SelectObjects(typeof(Email), filter.GetPredicate()));
            List<models.Email> emails = ModelAssembler.CreateEmails(businessCollection);
            businessCollection = new BusinessCollection(DataFacadeManager.Instance.GetDataFacade().SelectObjects(typeof(EmailType)));
            ...
            stopWatch.Stop();
            Debug.WriteLine(stopWatch.Elapsed.ToString(), "Sistran.Core.Application.UniquePersonService.DAOs.GetEmailsByIndividualId");
            return emails;
        }
        ...
        public models.Email CreateEmail(models.Email email, int individualId)
        {
            ...
            DataFacadeManager.Instance.GetDataFacade().InsertObject(emailEntity);
        ...
        public models.Email UpdateEmail(models.Email email, int individualId)
        {
            ...
            if (emails.Count > 0)
            {
                ...
                emailEntity = (Email)DataFacadeManager.Instance.GetDataFacade().GetObjectByPrimaryKey(key);
                emailEntity.EmailTypeCode = email.EmailType.Id;
                emailEntity.Address = email.Description;
                emailEntity.IsMailingAddress = email.IsMailingAddress;
                DataFacadeManager.Instance.GetDataFacade().UpdateObject(emailEntity);
                return ModelAssembler.CreateEmail(emailEntity);
            }
            else
            {
