...
namespace Sistran.Core.Application.UniquePersonService.DAOs
{
    ...
    public class CompanyDAO
    {
        ...
        /// 4. Todos</param>
        /// <returns></returns>
        public virtual List<Models.Company> GetCompaniesByDocumentNumberNameSearchType(string documentNumber, string name, int searchType, int? documentType, int? individualId)
        {
            ...
            BusinessCollection businessCollectionDoc = new BusinessCollection(DataFacadeManager.Instance.GetDataFacade().SelectObjects(typeof(UniquePerson.Entities.DocumentType)));
            ...
            {
                BusinessCollection<UniquePerson.Entities.Company> businessCollection;
                filter = new ObjectCriteriaBuilder();
                if (documentNumber != "")
                {
                    filter.Property(UniquePerson.Entities.Company.Properties.TributaryIdNo, typeof(UniquePerson.Entities.Company).Name);
                    filter.Equal();
                ...
                {
                    filter.Property(UniquePerson.Entities.Company.Properties.TradeName, typeof(UniquePerson.Entities.Company).Name);
                    filter.Like();
                    ...
                    using (IDataFacade dataFacade = DataFacadeManager.Instance.GetDataFacade())
                    {
                        string[] sort = { };
                        int totalCount;
                        businessCollection = dataFacade.List<UniquePerson.Entities.Company>(filter.GetPredicate(), sort, 200, 0, out totalCount);
            ...
            else if (searchType == (int)IndividualSearchType.ProspectusCompany || searchType == (int)IndividualSearchType.All)
            {
                ...
                if (documentNumber != "")
                {
                    filter.Property(UniquePerson.Entities.Prospect.Properties.TributaryIdNo, typeof(UniquePerson.Entities.Prospect).Name);
                    filter.Equal();
                    filter.Constant(documentNumber);
                    businessCollection1 = new BusinessCollection(DataFacadeManager.Instance.GetDataFacade().SelectObjects(typeof(UniquePerson.Entities.Prospect), filter.GetPredicate()));
                }
                else
                {
                    filter.Property(UniquePerson.Entities.Prospect.Properties.TradeName, typeof(UniquePerson.Entities.Prospect).Name);
                    filter.Like();
                    filter.Constant(name + "%");
                    ...
                    using (IDataFacade dataFacade = DataFacadeManager.Instance.GetDataFacade())
                    {
                        ...
                        businessCollection1 = dataFacade.List<UniquePerson.Entities.Prospect>(filter.GetPredicate(), sort, 200, 0, out totalCount);

                        if (totalCount == 1)
                        {
                            businessCollection1 = DataFacadeManager.Instance.GetDataFacade().List<UniquePerson.Entities.Prospect>(filter.GetPredicate());
                        }
            ...
            stopWatch.Stop();
            Debug.WriteLine(stopWatch.Elapsed.ToString(), "Sistran.Core.Application.UniquePersonService.DAOs.GetCompaniesByDocumentNumberNameSearchType");
            return companiesAll;
        }
        ...
        public virtual Models.Company GetCompanyByIndividualId(int individualId)
        {
            Stopwatch stopWatch = new Stopwatch();
            stopWatch.Start();
            PrimaryKey key = UniquePerson.Entities.Company.CreatePrimaryKey(individualId);
            UniquePerson.Entities.Company companyEntity = (UniquePerson.Entities.Company)DataFacadeManager.Instance.GetDataFacade().GetObjectByPrimaryKey(key);
            ...
            if (companyEntity != null)
            {
                ...
                {
                    company.LegalRepresentative.ContactName = companyEntity.ContactName;
                    company.LegalRepresentative.ManagerName = companyEntity.ManagerName;
                    company.LegalRepresentative.GeneralManagerName = companyEntity.GeneralManagerName;
                    company.LegalRepresentative.ContactAdditionalInfo = companyEntity.ContactAdditionalInfo;
                }

            }

            stopWatch.Stop();
            Debug.WriteLine(stopWatch.Elapsed.ToString(), "Sistran.Core.Application.UniquePersonService.DAOs.GetCompanyByIndividualId");
            return company;
        }
        ...
        //    filter.Equal();
        //    filter.Constant(documentNumber);
        //    BusinessCollection businessCollection = new BusinessCollection(DataFacadeManager.Instance.GetDataFacade().SelectObjects(typeof(UniquePerson.Entities.Company), filter.GetPredicate()));
        //    Models.Company company = ModelAssembler.CreateCompanies(businessCollection).FirstOrDefault();

        //    stopWatch.Stop();
        //    Debug.WriteLine(stopWatch.Elapsed.ToString(), "Sistran.Core.Application.UniquePersonService.DAOs.GetCompanyByDocumentNumber");
        //    return company;
        //}
        ...
        public virtual Models.Company CreateCompany(Models.Company company)
        {
            ...
            using (Context.Current)
            {
                using (Transaction transaction = new Transaction())
                {
                    ...
                    try
                    {
                        company.IndividualType = Enums.IndividualType.LegalPerson;
                        UniquePerson.Entities.Company companyEntity = EntityAssembler.CreateCompany(company);

                        DataFacadeManager.Instance.GetDataFacade().InsertObject(companyEntity);

                        if (company.Addresses != null)
                        {
                            foreach (Models.Address Ad in company.Addresses)
                            {
                                addressDAO.CreateAddress(Ad, companyEntity.IndividualId);
                            }
                        }
                        if (company.Phones != null)
                        {
                            foreach (Models.Phone phon in company.Phones)
                            {
                                phoneDAO.CreatePhone(phon, companyEntity.IndividualId);
                            }
                        }

                        if (company.Emails != null)
                        {
                            foreach (Models.Email em in company.Emails)
                            {
                                emailDAO.CreateEmail(em, companyEntity.IndividualId);
                            }
                        }

                        if (company.PaymentMethodAccount != null)
                        {
                            foreach (Models.PaymentMethodAccount pmac in company.PaymentMethodAccount)
                            {
                                paymentMethodDAO.CreatePaymentMethod(pmac, companyEntity.IndividualId);
                                if (pmac.PaymentMethod.Id != (int)PaymentMethodType.Cash)
                                {
                                    paymentMethodAccountDAO.CreatePaymentMethodAccount(pmac, companyEntity.IndividualId);
                                }
                        ...
                        if (company.LegalRepresentative != null)
                        {
                            legalRepresentativeDAO.CrateRepresentLegal(company.LegalRepresentative, companyEntity.IndividualId);
                        }

                        if (company.Partners != null)
                        {
                            foreach (Models.Partner prt in company.Partners)
                            {
                                partnerDAO.CreatePartner(prt, companyEntity.IndividualId);
                            }
                        }

                        if (company.CompanyExtended != null)
                        {
                            CompanyExtendedDAO coCompanyDAO = new CompanyExtendedDAO();
                            coCompanyDAO.CreateCoCompany(company.CompanyExtended, companyEntity.IndividualId);
                            if (company.CompanyExtended.AssociationType.Id == (int)AssociationTypes.FutureCompany)
                            {
                                CommonService.Models.Parameter parameter = new CommonService.Models.Parameter();
                                parameter.Id = (int)Sistran.Core.Application.CommonService.Enums.ParametersTypes.FutureSociety;
                                parameter.NumberParameter = Convert.ToInt32(company.IdentificationDocument.Number) + 1;
                                DelegateService.commonServiceCore.UpdateParameter(parameter);
                            }
                        }
                        if (company.CompanyNames != null)
                        {
                            foreach (Models.CompanyName cn in company.CompanyNames)
                            {
                                coCompanyNameDAO.CreateCompaniesName(cn, companyEntity.IndividualId);
                            }

                        }
                        if (company.Roles != null)
                        {
                            individualRolDAO.CreateIndividualRoles(company.Roles, company.IndividualId);
                        }
                        transaction.Complete();

                        stopWatch.Stop();
                        Debug.WriteLine(stopWatch.Elapsed.ToString(), "Sistran.Core.Application.UniquePersonService.DAOs.CreateCompany");
        ...
        public virtual Models.Company UpdateCompany(Models.Company company)
        {
            ...
            PrimaryKey key = UniquePerson.Entities.Company.CreatePrimaryKey(company.IndividualId);
            UniquePerson.Entities.Company companyEntity = (UniquePerson.Entities.Company)DataFacadeManager.Instance.GetDataFacade().GetObjectByPrimaryKey(key);

            if (companyEntity != null)
            {
                companyEntity.EconomicActivityCode = company.EconomicActivity.Id;
                companyEntity.TradeName = company.Name;
                ...
                companyEntity.CountryCode = company.CountryOrigin.Id;
                companyEntity.CompanyTypeCode = company.CompanyType.Id;

                if (company.LegalRepresentative != null)
                {
                    companyEntity.ContactName = company.LegalRepresentative.ContactName;
                    companyEntity.ManagerName = company.LegalRepresentative.ManagerName;
                    companyEntity.GeneralManagerName = company.LegalRepresentative.GeneralManagerName;
                    companyEntity.ContactAdditionalInfo = company.LegalRepresentative.ContactAdditionalInfo;
                    legalRepresentativeDAO.UpdateLegalRepresent(company.LegalRepresentative, company.IndividualId);
                }
                ...
                if (company.Addresses != null)
                {
                    foreach (Models.Address Ad in company.Addresses)
                    {
                        if (Ad.Id == 0)
                        {
                            addressDAO.CreateAddress(Ad, company.IndividualId);
                        }
                        else
                        {
                            addressDAO.UpdateAddress(Ad, company.IndividualId);
                        }
                    }
                }
                ...
                if (company.Phones != null)
                {
                    foreach (Models.Phone phon in company.Phones)
                    {
                        if (phon.Id == 0)
                        {
                            phoneDAO.CreatePhone(phon, company.IndividualId);
                        }
                        else
                        {
                            phoneDAO.UpdatePhone(phon, company.IndividualId);
                        }
                    }
                }
                ...
                if (company.Emails != null)
                {
                    foreach (Models.Email em in company.Emails)
                    {
                        if (em.Id == 0)
                        {
                            emailDAO.CreateEmail(em, company.IndividualId);
                        }
                        else
                        {
                            emailDAO.UpdateEmail(em, company.IndividualId);
                        }
                    }
                }
                ...
                if (company.Partners != null)
                {
                    foreach (Models.Partner pms in company.Partners)
                    {
                        if (pms.PartnerId == 0)
                        {
                            partnerDAO.CreatePartner(pms, company.IndividualId);
                        }
                        else
                        {
                            partnerDAO.UpdatePartner(pms, company.IndividualId);
                        }
                    }
                }
                ...
                {
                    CompanyExtendedDAO coCompanyDAO = new CompanyExtendedDAO();
                    coCompanyDAO.UpdateCoCompany(company.CompanyExtended, company.IndividualId);
                    if (company.CompanyExtended.AssociationType.Id == (int)AssociationTypes.FutureCompany)
                    {
                        CommonService.Models.Parameter parameter = new CommonService.Models.Parameter();
                        parameter.Id = (int)Sistran.Core.Application.CommonService.Enums.ParametersTypes.FutureSociety;
                ...
                if (company.PaymentMethodAccount != null)
                {
                    foreach (Models.PaymentMethodAccount pmac in company.PaymentMethodAccount)
                    {
                        if (pmac.PaymentMethod.Id != (int)PaymentMethodType.Cash)
                        {
                            paymentMethodAccountDAO.CreatePaymentMethodAccount(pmac, company.IndividualId);
                        }
                ...
                if (company.CompanyNames != null)
                {
                    foreach (Models.CompanyName cnn in company.CompanyNames)
                    {
                        coCompanyNameDAO.UpdateCompaniesName(cnn, company.IndividualId);
                    }
                }
                if (company.Roles != null)
                {
            ...
            stopWatch.Stop();
            Debug.WriteLine(stopWatch.Elapsed.ToString(), "Sistran.Core.Application.UniquePersonService.DAOs.UpdateCompany");
            return GetCompanyByIndividualId(companyEntity.IndividualId);
        }
    }
}