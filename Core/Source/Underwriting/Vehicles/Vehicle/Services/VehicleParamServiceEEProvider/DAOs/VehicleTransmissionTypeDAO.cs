...
namespace Sistran.Core.Application.VehicleParamService.EEProvider.DAOs
{
    ...
    /// </summary>
    public class VehicleTransmissionTypeDAO
    {
        /// <summary>
        /// Obtiene la lista de Versiones de Carrocerias del vehiculo.
        /// </summary>
        /// <returns>Lista de Versiones de Carrocerias consultadas</returns>
        public Result<List<ParamVehicleTransmissionType>, ErrorModel> GetParamVehicleTransmissionType()
        {
            ...
            try
            {
                ...
                Result<List<ParamVehicleTransmissionType>, ErrorModel> listParamVehicleVersion = ModelAssembler.CreateParamVehicleTransmissionType(businessCollection);
                if (listParamVehicleVersion is ResultError<List<ParamVehicleTransmissionType>, ErrorModel>)
                {
                    return listParamVehicleVersion;
                }
                else
                {
                    List<ParamVehicleTransmissionType> resultValue = (listParamVehicleVersion as ResultValue<List<ParamVehicleTransmissionType>, ErrorModel>).Value;
                    return listParamVehicleVersion;
                }
            }
            catch (Exception ex)
            {
                errorModel.Add(Errors.ErrorQueryVehicleTransmissionType);
                return new ResultError<List<ParamVehicleTransmissionType>, ErrorModel>(ErrorModel.CreateErrorModel(errorModel, ErrorType.TechnicalFault, ex));
            }
        }
    }
}
