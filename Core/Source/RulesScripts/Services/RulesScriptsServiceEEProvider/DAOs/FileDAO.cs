...
//using global::Core.UtilitiesServicesEEProvider.Assemblers;
...
namespace Sistran.Core.Application.RulesScriptsServices.EEProvider.DAOs
{
    ...
    {
        /// <summary>
        /// Exporta paquete de reglas a excel de acuerdo a filtro 
        /// </summary>
        /// <param name="rules">paquete de reglas a exportar</param>        
        /// <returns>archivo de excel </returns>        
        public string GenerateFileToRules(List<MRules._RuleSet> rules)
        {
            try
            {
                string fileName = Resources.Errors.FileNameRules;
                FileProcessValue fileProcessValue = new FileProcessValue()
                {
                    Key1 = (int)FileProcessType.Rules
                };
                ...
                if (file != null && file.IsEnabled)
                {
                    ...
                    foreach (MRules._RuleSet item in rules)
                    {
                        ...
                        fields[0].Value = item.RuleSetId.ToString();
            ...
            {
                throw new BusinessException("Error al generar excel", ex);
            }
        }
        ...
        public string GenerateFileToDecisionTableByExceptions(List<string[]> exceptions, string mensajeArchivo= "Tabla_de_decisión_con_errores")
        {
            try
            {
                FileDAO fileDAO = new FileDAO();
                ...
                List<string[]> excelConfiguration = new List<string[]>();
                excelConfiguration.Add(new string[] { "1", "Regla" });
                excelConfiguration.Add(new string[] { "2", "Error" });

                foreach (string[] item in exceptions)
                {
                    var fields = excelConfiguration.Select(x => new Field
                    {
                        ColumnSpan = 1,
                        Description = x[1],
                        ...
                        IsEnabled = true,
                        IsMandatory = true,
                        Order = Convert.ToInt32(x[0]),
                        RowPosition = 1,
                        SmallDescription = x[1]
                    }).ToList();

                    fields[0].Value = item[0];
                    fields[1].Value = item[1];
                    ...
                    file.Templates = new List<Template>() { new Template() { Rows = rows} };
                    file.Name = mensajeArchivo + "_" + DateTime.Now.ToString("dd_MM_yyyy");
                    file.Description= Resources.Errors.FileNameRules;
                    file.SmallDescription = Resources.Errors.FileNameRules;
                    file.FileType = FileType.Excel;
                    file.IsEnabled = true;
                    file.Templates[0].Description = Resources.Errors.FileNameRules;
                    file.Templates[0].Order = 1;
                    file.Templates[0].IsMandatory = true;
                    file.Templates[0].IsPrincipal = true;
                    return fileDAO.GenerateFile(file);
